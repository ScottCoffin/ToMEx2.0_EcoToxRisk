...
validate_rtrunc_args <- function(dist, minv, maxv, mode = NULL, lsup, linf) {
  ...
  if (lsup <= linf) {
    stop("lsup must be greater than linf.")
  }
  if (lsup < minv) {
    stop("lsup must be >= min.")
  }
  if (linf > maxv) {
    stop("linf must be <= max.")
  }
  if (dist == "rtriang") {
    if (is.null(mode)) {
      stop("mode must be provided for rtriang.")
    }
    if (!is.numeric(mode) || is.na(mode) || is.nan(mode)) {
      stop("mode must be numeric and not NA/NaN.")
    }
    if (minv > mode || mode > maxv) {
      stop("min <= mode <= max must hold for rtriang.")
    }
...
rmore <- function(
  ...
  for (i in 2:(length(uni.values) - 1)) {
    height.values[i] <- max(
      freq.uni.values[i] * N / (uni.values[i + 1] - uni.values[i]),
  ...
  height.values[length(height.values)] <-
    freq.uni.values[length(height.values)] *
    N /
    (uni.values[length(height.values)] -
      uni.values[length(height.values) - 1])

  # LEFT TRIANGLE
  if (
    (length(unique(sort.values)) < length(sort.values)) &
  ...
  for (i in 1:(length(uni.values) - 1)) {
    if (height.values[i] == 1 & height.values[i + 1] == 1) {
      ...
      max.trunc.distr <- (h1 * uni.values[i + 1] - h2 * uni.values[i]) /
        (h1 - h2)
      A <- ((max.trunc.distr - uni.values[i]) *
        h1 -
        (max.trunc.distr - uni.values[i + 1]) * h2) /
        2
      validate_rtrunc_args(
        "rtriang",
        uni.values[i],
      ...
      h1 <- height.values[i]
      h2 <- height.values[i + 1]
      min.trunc.distr <- (h2 * uni.values[i] - h1 * uni.values[i + 1]) /
        (h2 - h1)
      A <- ((uni.values[i + 1] - min.trunc.distr) *
        h2 -
        (uni.values[i] - min.trunc.distr) * h1) /
        2
      validate_rtrunc_args(
        "rtriang",
        min.trunc.distr,
  ...
  if (
    ...
    A <- N *
      (dist.max - val.max) /
      (2 * (val.max - sort.values[length(sort.values) - 1]))
    validate_rtrunc_args(
      "rtriang",
      sort.values[length(sort.values)],
      dist.max,
      sort.values[length(sort.values)],
      lsup,
      linf
