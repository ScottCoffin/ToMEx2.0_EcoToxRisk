...
library(dplyr)
library(pals)
# load boxplot function
source("scripts/characteristics and NMDS/XX_Helper_boxplot_Kooi2021.R")

# import prepared ToMEx2.0 data (originally from ToMEx 2 repo)
dat = readRDS("data/output/aoc_final.RDS")

# import data from Kooi et al. 2021:
dat.lengths = read.csv(
        "data/input/characteristics and NMDS/XX_KooiEtAl2021_particle_length.csv"
)
...
dat$particleID = paste0(
        dat$doi,
        dat$poly_f,
        dat$shape_f,
        dat$charge,
        dat$size.length.um.used.for.conversions,
        dat$functional.group,
        dat$weather.biofoul_f
)
dat$treatmentID = paste0(
        dat$particleID,
        dat$Species,
        dat$life_f,
        dat$vivo_f,
...
barplot(
        polymer2,
        legend = rownames(polymer2),
        col = polychrome(30), #col = hcl.colors(30, "RdYlGn"),
...
boxplot_comp(
        treatdat$size.length.um.used.for.conversions,
        tomex_color = "darkorange2",
        dat.lengths,
        c(
                "palegreen",
                "cornsilk3",
...
barplot(
        shape2,
        legend = rownames(shape2),
        col = c("grey60", "royalblue", "darkolivegreen3")
)
