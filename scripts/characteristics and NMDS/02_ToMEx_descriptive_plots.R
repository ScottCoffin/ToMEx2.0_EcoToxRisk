...
# Import data
dat <- readRDS("data/output/characteristics and NMDS/prepared_data.RDS")
...
# Polymer ----
polyfresh <- fresh %>% group_by(poly_group) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
polymarine <- marine %>% group_by(poly_group) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
polymerged <- merge(polyfresh, polymarine, by = "poly_group", sort = FALSE, all.x = TRUE)
polymerged <- polymerged %>% arrange(freq.x) %>%  mutate(poly_group = factor(poly_group, levels = poly_group))
...
# Particle Shape ----
shapefresh <- fresh %>% group_by(shape_f) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
shapemarine <- marine %>% group_by(shape_f) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
shapemerged <- merge(shapefresh, shapemarine, by = "shape_f", sort = FALSE)
shapemerged <- shapemerged %>% arrange(freq.x)

shapemerged$shape_f <- factor(shapemerged$shape_f,
                              levels = shapemerged$shape_f)
...
# Sodium azide ----
sodfresh <- fresh %>% group_by(sodium.azide) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
sodmarine <- marine %>% group_by(sodium.azide) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
...
# DOM ----
domfresh <- fresh %>% group_by(DOM_present) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
dommarine <- marine %>% group_by(DOM_present) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
...
# Surface charge ----
chargefresh <- fresh %>% group_by(charge) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
chargemarine <- marine %>% group_by(charge) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
...
# Functional group ----
functfresh <- fresh %>% group_by(functional.group) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
functmarine <- marine %>% group_by(functional.group) %>% dplyr::summarize(count = n()) %>% mutate(freq = count / sum(count))
...
final_plot
# Save the final plot
ggsave("output/Manuscript_Figs/characteristics and NMDS/figureS12_descriptive_plots.png",
       plot = final_plot, width = 30, height = 20, units = "cm", dpi = 500)


#####################################################################
## Base R Approach ##################################################
## GGplot method used in final paper. Code below kept for reference.#
#####################################################################
# #detach("package:plyr")
# library(dplyr)
# library(pals)
...
#     mutate(freq = count/sum(count)))
# polymerged = merge(polyfresh, polymarine, by = "poly_group", sort = FALSE, all.x = TRUE)
# (polymerged = polymerged %>% arrange(desc(freq.x))
# 
...
#     mutate(freq = count/sum(count)))
# sodmerged = merge(sodfresh, sodmarine, by = "sodium.azide", sort = FALSE)
# (sodmerged = sodmerged %>% arrange(desc(freq.x))
# 
...
# (chargemerged = chargemerged %>% arrange(desc(freq.x))
...
# 
# #png("output/Manuscript_Figs/characteristics and NMDS/descriptive_plots.png",
