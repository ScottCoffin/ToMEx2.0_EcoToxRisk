...
CleanFile <- ToMExFinal %>%
  ...
  filter(
    ...
    risk.13 != 0
  ) %>% #Drop studies that received a score of 0 for endpoints criteria (this also drops studies that have not yet been))
  mutate(
    bio_f = recode(
      bio_f,
      'subcell' = 'Subcell',
...
CleanFile$bio_f <- factor(
  CleanFile$bio_f,
  levels = c('Subcell', 'Cell', 'Tissue', 'Individual', 'Population')
)
...
Figure <- CleanFile %>%
  ...
  geom_bar(
    stat = "count", #need to add 1 to shown 1's
  ...
  facet_grid(
    ...
    scales = "free_y"
  ) + # Allow y-axis to vary by facet
  labs(x = "Data Points", y = NULL, title = NULL) +
...
ggsave(
  "output/Manuscript_Figs/figure2_bio_response_taxa.jpg",
  Figure,
  width = 7,
  height = 5,
  dpi = 400
