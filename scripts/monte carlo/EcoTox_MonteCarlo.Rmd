...
# TomEx 1.0 data file prepped in ToMEx2.0_AquaticOrganisms repo (main folder)
...
# tomex2.0 <- read_rds("../../data/input/aoc_z_tomex2.rds") %>% 
...
```{r}
formatted_thresholds <- read.csv("../../data/output/formatted_thresholds.csv")
...
# uncomment unless running script to save results!
```
```{r}
sobol_time
```
...
results_df_sobol <- do.call(rbind, lapply(MC_results_tox_sobol, function(x) {
  ...
  mutate(
    ...
    translocatable = case_when(
      ...
      T ~ translocatable)) #%>% 
  # join with metadata
    # left_join(aoc_aligned_test %>%
    #           select(af.noec, af.time, unique_id, poly_f, shape_f, environment, effect.metric, bio_f, 
    #                  risk.13, tier_zero_tech_f, tier_zero_risk_f) %>% 
    #             rename(polymer = poly_f, shape = shape_f) %>% 
    #           mutate(unique_id = as.factor(unique_id)),
    #         by = "unique_id"
    #         )
...
cat(crayon::green("MC-Sim Script finished!"),
    "Now make sure you run the ToMEx2_EcoTox.Rmd script now.")
```
