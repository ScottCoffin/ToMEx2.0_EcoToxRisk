...
model_wrapper_sobol_parallel <- function(params) {
  ...
  marine_thresholds <- process_environment_data(aoc_risk_paper,
    ...
    convert_units_fxn(environment = "Marine", params = params)
  ...
  freshwater_thresholds <- process_environment_data(aoc_risk_paper,
                                                    ...
                                                    x2D_set = x2D_set) %>% 
    ## convert particles/L thresholds to volume, mass, surface area (ERM-dependent)
    convert_units_fxn(environment = "Freshwater", params = params)
...
```{r}
results_df_sobol %>% 
  left_join(aoc_aligned_test, by = "unique_id") %>% 
...
results_df_sobol %>% 
  ...
  group_by(ingestible) %>% 
  summarize(n = n()) %>% 
  mutate(n = n / results_df_sobol$n_iterations[1])
```
...
#define limits
min_val <- min(aoc_MC_sobol$CoV_food_dilution, na.rm = TRUE)
max_val <- max(aoc_MC_sobol$CoV_food_dilution, na.rm = TRUE)
