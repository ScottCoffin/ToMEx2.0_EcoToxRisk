...
```{r}
#rm(list = ls())
# load custom SSD and alignment functions
...
source("../utils/RDAmaker_functions.R") #functionalized data tidying script for ToMEx 1.0 data
source("../utils/ToMEx2.0_Data_Tidying_functions.R") #functionalized data tidying script for ToMEx 2.0 data
```
...
thresholds_df %>% 
  ...
  mutate(tissue_translocation_um2.L =  `Tissue Translocation (Default)` * mux_polyfnx(
  ...
  mutate(tissue_translocation_ug.L =  `Tissue Translocation (Default)` *mux_polyfnx(
    a.x = a.m.input, #alpha for marine surface water for mass
    x_UL = x2D_set, #upper limit of default range
    x_LL = x1D_set # lower limit of default range
                                       )) %>% 
  ## convert food dilution thresholds
  mutate(food_dilution_um3.L =  `Food Dilution (Default)` * mux_polyfnx
         (a.x = a.v.input, #alpha for marine surface water for surface area
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) %>% 
  mutate(food_dilution_ug.L =  `Food Dilution (Default)` * mux_polyfnx(
    a.x = a.m.input, #alpha for marine surface water for mass
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) 
...
# TomEx 1.0 data file prepped in ToMEx2.0_AquaticOrganisms repo (main folder) (https://github.com/SCCWRP/ToMEx_AquaticOrganisms/blob/master/aoc_setup.RDS)
aoc <- read_rds("../../data/input/aoc_setup.RDS")
...
bodysize_addons <- read_csv("ref data/gape_size.csv", show_col_types = FALSE) #copied from aq_mp_tox_shiny main folder (https://github.com/SCCWRP/ToMEx_AquaticOrganisms/blob/master/gape_size.csv)
```
...
upper.tissue.trans.size.um <- 88 #10 #um #set size for x2M #only used for determinstic assessment
```
...
```{r}
## Distribution values from https://doi.org/10.1016/j.watres.2021.117429
# particle properties
...
H_W_ratio.marine_samples <- R.ave.water.marine_samples
...
H_W_ratio.freshwater_samples <- R.ave.water.freshwater_samples
...
param_values <- data.frame(
  ...
  H_W_ratio.marine_samples = H_W_ratio.marine_samples,
  ...
  H_W_ratio.freshwater_samples = H_W_ratio.freshwater_samples,
...
MC_param_histograms_base <- param_values %>% 
  ...
  mutate(
    ...
    name = factor(name, levels = c("alpha.marine", "a.sa.marine", "a.v.marine",
                                        "a.m.marine", "a.ssa.marine", "R.ave.water.marine",
                                        "H_W_ratio.marine", "H_W_ratio.freshwater",
...
```{r}
write.csv(param_values, "../../data/output/parameter_values.csv")
param_values
```
...
TEST to make sure produces same df as in deterministic thresholds.

```{r message=FALSE, warning=FALSE}
aoc_aligned_test <- align_data(aoc_risk_paper) %>% 
  ...
  aoc_aligned <- aoc_aligned_test 
    
  
  # calculate thresholds for different environments
marine_thresholds <- process_environment_data(aoc_aligned,
                                                "Marine", 
                                                upper.tissue.trans.size.um = upper.tissue.trans.size.um,
                                                x1D_set = x1D_set, 
                                                x2D_set = x2D_set)
  
freshwater_thresholds <- process_environment_data(aoc_aligned, 
                                                    "Freshwater",
                                                    upper.tissue.trans.size.um = upper.tissue.trans.size.um,
                                                    x1D_set = x1D_set, 
                                                    x2D_set = x2D_set)
...
formatted_marine_base <- marine_thresholds %>% 
...
formatted_freshwater_base <- freshwater_thresholds %>% 
  mutate_if(is.numeric, ~signif(., 2)) %>% 
  mutate(across(where(is.numeric), ~scales::comma(.))) %>% 
...
formatted_thresholds_montecarlo <- cbind(formatted_marine_base, formatted_freshwater_base)

formatted_thresholds_montecarlo
```


Deterministic thresholds derived in ToMEx2_EcoTox.Rmd
```{r}
formatted_thresholds <- read_csv("../../data/output/formatted_thresholds.csv", )
formatted_thresholds
```


#### Compare deterministic aligned dataset and MC-aligned (using default parameters)

MC threshold dataset
```{r}
aoc_aligned %>% 
      filter(Group != "Algae") %>%
...
Deterministic threshold dataset prepared in ToMEx2_EcoTox.Rmd
```{r}
marine_alpha_thresholds <- readRDS("../../data/output/marine_alpha_thresholds.rds")
...
MC_histograms_fxn <- function(long_data = all_thresholds_long,
                              environment = "Marine",
                              n_bins = round(n_sim *0.8),
                              units_input = "Particles/L",
                              #define custom limits for FD and tissue trans
                              food_limits = c(0.0001, 500),
                              tissue_limits = c(05, 25000),
...
return(list(MC_histograms_light#,
            #MC_histograms_dark
            ))
}
```
...
aoc_MC_iter <- align_data(aoc_aligned,
  ...
  freshwater_thresholds <- process_environment_data(aoc_aligned,
   ...
   # Store results in list
    sobol_result <- list(
      ...
      unique_id = aoc_aligned$unique_id), #row id that allows matching back to database
...
n_sobol <- 589 #times 16 is 9,424

matrices <- c("A", "B", "AB", "BA")
first <- total <- "azzini"

params <- c("alpha.marine", "a.sa.marine", "a.v.marine", "a.m.marine", 
            "alpha.freshwater", "a.sa.freshwater", "a.v.freshwater", "a.m.freshwater", 
            "R.ave.water.marine", "R.ave.water.freshwater", "H_W_ratio.marine", "H_W_ratio.freshwater",
...
mat[, H_W_ratio.marine := rtruncnorm(.N, a = 0.0001, b= 1, mean = R_ave.water.marine, sd = R.ave.water.marine.sd)]
mat[, R.ave.water.freshwater := rtruncnorm(.N, a = 0.0001, b= 1, mean = R_ave.water.freshwater, sd = R.ave.water.freshwater.sd)]
mat[, H_W_ratio.freshwater := rtruncnorm(.N, a = 0.0001, b= 1, mean = R_ave.water.freshwater, sd = R.ave.water.freshwater.sd)]
...
upper.tissue.trans.size.um_samples <- sim_X50 %>% 
  ...
  filter(. < 500) %>% #truncate distribution below biologically plausible limit
  slice(1:nrow(mat))
...
### not used as these are produced in ToMEx2_EcoTox.Rmd at journal quality
# ggsave(filename = "MC_param_histograms.jpg",
#        dpi = 300,
#        path = "../../output/Manuscript_Figs/", 
...
translocation_data <- mat %>%
  ...
  rename("Maximum Translocatable Particle Length (µm)" = "upper.tissue.trans.size.um") %>%
  pivot_longer(cols = everything(), names_to = "name", values_to = "value") %>%
...
ggsave(filename = "figure1_a_alpha_combined_plot.jpg",
       dpi = 300,
       path = "../../output/Manuscript_Figs/", 
       plot = alpha_combined_plot, width = 9, height = 5, units = "in")

ggsave(filename = "figure1_b_tissue_body.jpg",
       dpi = 300,
       path = "../../output/Manuscript_Figs/", 
...
# check if sobol_results already exists to skip running
if (file.exists("output/sobol_results.rds")){
  sobol_results <- readRDS("output/sobol_results.rds")
}

if (file.exists("output/sobol_results.rds")) {
  cat(crayon::green("sobol_results already exists. Skipping Sobol analysis."))
} else {
  cat(crayon::yellow("Running Sobol analysis..."))
...
sobol_results <- foreach(i = 1:n_row_sobol,
                                  ...
                                  .options.snow = opts,
                                  .export = c("model_wrapper_sobol_parallel",
                                              "aoc_aligned",
                                              "align_data",
                                              "filter_environment_data", "generate_structure_checks",
                                              "mux_polyfnx", "CFfnx",
                                              "process_environment_data",  "sensitivity_t1",
                                              "SSD_function_t1", "SSD_function_t2",
                                              "SSD_function_t3_4", "summarize_and_print", "update_particle_length",
                                              "volumefnx", "SAfnx", "massfnx", "SSA.inversefnx",
                                              "n_sim",
                                              "x1D_set", "x2D_set",
                                              "mat"),
                         .combine = rbind) %dopar% {
...
cat(crayon::green(paste0("Sobol analysis completed in ", sobol_time)))
...
cat(crayon::green("Sobol analysis completed and results saved to 'output/sobol_results.rds'"))
}
```
...
all_thresholds_sobol_combined <- bind_rows(
  ...
  mutate(all_thresholds_freshwater_sobol, Environment = "Freshwater"))
...
all_thresholds_sobol_long <- all_thresholds_sobol_combined %>%
  ...
  filter(Value > 0) #remove invalid values


# Step 3: Filter for the metrics of interest
metrics_of_interest <- c("Tissue Translocation (Default)", "Food Dilution (Default)")
thresholds_filtered <- all_thresholds_sobol_long %>%
  filter(Metric %in% metrics_of_interest)

# Step 4: Define a function to calculate z-scores
...
# Step 5: Calculate z-scores for each Tier, Metric, and Environment
z_scores <- thresholds_filtered %>%
  group_by(Tier, Metric, Environment) %>%
  mutate(Z_Score = calculate_z_scores(Value)) %>%
  ungroup() 

# Step 6: Identify outliers based on z-scores (e.g., |z| > 3)
outliers <- z_scores %>%
  filter(abs(Z_Score) > 3)

# Step 7: Get the simulation IDs of outliers
outlier_simulation_ids <- outliers %>%
  pull(simulation_id) %>%
  unique()

# Step 8: Remove outliers from `mat`, thresholds, and sobol results datasets
mat_filtered <- mat %>%
  filter(!simulation_id %in% paste0("sim",outlier_simulation_ids))

all_thresholds_sobol_filtered <- all_thresholds_sobol_combined %>%
  filter(!simulation_id %in% outlier_simulation_ids)

all_thresholds_sobol_long_filtered <- all_thresholds_sobol_long %>% 
  filter(!simulation_id %in% outlier_simulation_ids)

sobol_results_filtered <- sobol_results %>%
  keep(~ !(.x$simulation_id %in% outlier_simulation_ids))


# Step 9: Calculate percentage of filtered data for each group
z_scores_grouped <- z_scores %>% 
  mutate(outlier = case_when(abs(Z_Score) >3 ~ "outlier",
                             T ~ "not-outlier")) %>% 
  group_by(Tier, Metric, Environment, outlier) %>% 
  summarize(total_sim = n_distinct(simulation_id)) %>% 
  pivot_wider(values_from = total_sim,
              names_from = outlier) %>% 
  mutate(percent_outliers = paste0( signif( ((outlier / (`not-outlier` + outlier)) * 100), 2), "%")) %>% 
  distinct(Tier, Metric, Environment, percent_outliers) %>% 
  pivot_wider(names_from = c(Environment, Metric),
              values_from = percent_outliers)


# Step 10: Save the filtered data and percentage summary
saveRDS(mat_filtered, "output/mat_filtered.rds")
saveRDS(all_thresholds_sobol_filtered, "output/all_thresholds_sobol_filtered.rds")
write.csv(z_scores_grouped, "output/percent_filtered_summary.csv", row.names = FALSE)
saveRDS(all_thresholds_sobol_long_filtered, "output/all_thresholds_sobol_long_filtered.rds")
saveRDS(sobol_results_filtered, "output/sobol_results_filtered.rds")

## report percentage of simulations filtered for each group based on z-scores
z_scores_grouped
```

```{r}
sobol_results_filtered <- readRDS("output/sobol_results_filtered.rds")
all_thresholds_sobol_filtered <- readRDS("output/all_thresholds_sobol_filtered.rds")
all_thresholds_sobol_long_filtered <- readRDS("output/all_thresholds_sobol_long_filtered.rds")
...
Y_marine_food_T3 <- as.numeric(all_thresholds_sobol_filtered %>% filter(Tier == "Tier3", Environment == "Marine") %>% 
                                 pull(`Food Dilution (Default)`))

Y_freshwater_food_T3 <- as.numeric(all_thresholds_sobol_filtered %>% filter(Tier == "Tier3", Environment == "Freshwater") %>% 
                                     pull(`Food Dilution (Default)`))

Y_marine_tissue_T3 <- as.numeric(all_thresholds_sobol_filtered %>% filter(Tier == "Tier3", Environment == "Marine") %>% 
                                   pull(`Tissue Translocation (Default)`))
Y_freshwater_tissue_T3 <- as.numeric(all_thresholds_sobol_filtered %>% filter(Tier == "Tier3", Environment == "Freshwater")  %>% 
                                       pull(`Tissue Translocation (Default)`))
...
```{r}
summary_stats_base_thresholds_sobol <- as.tibble(all_thresholds_sobol_long_filtered) %>% 
...
```{r}
min_max_sobol <- all_thresholds_sobol_long_filtered %>%
...
```{r}
### Generate plots
Marine_histograms_sobol <- MC_histograms_fxn(all_thresholds_sobol_long_filtered, 
                                             "Marine",
...
Freshwater_histograms_sobol <- MC_histograms_fxn(all_thresholds_sobol_long_filtered,
                                                 "Freshwater",
                                                 n_bins = round(n_sobol *0.5),
                                                 units_input = "Particles/L",
...
##### ug/L
```{r}
min_max_sobol_mass <- all_thresholds_sobol_long_filtered %>%
...
```{r}
### Generate plots
Marine_histograms_sobol_mass <- MC_histograms_fxn(all_thresholds_sobol_long_filtered, "Marine",
                                          n_bins = round(n_sobol * 0.5),
                                          units_input = "ug/L",
                                          x_axis_input = element_blank(),
...
Freshwater_histograms_sobol_mass <- MC_histograms_fxn(all_thresholds_sobol_long_filtered, "Freshwater",
                                              n_bins = round(n_sobol *0.5),
                                              units_input = "ug/L",
                                              x_axis_input = element_text(),
                                              food_limits = c(signif((min_max_sobol_mass$min[1] * 1),1),
...
tier_colors <- setNames(c("#b8e897", "#f0f062", "#f0a95f", "#f0514b"),
                        levels(as.factor(all_thresholds_sobol_long_filtered$Tier)))

# Calculate the 99th percentile for each group to identify outliers
outliers <- all_thresholds_sobol_long_filtered %>%
...
all_thresholds_violin_sobol <- all_thresholds_sobol_long_filtered %>% 
  filter(grepl("Default", Metric)) %>%
  mutate(ERM = case_when(
  ...
  scale_x_log10(
    name = "Particles/L (1 - 5000 µm particles)",
  # labels = scales::label_number(accuracy = 0.01, trim = TRUE),
    breaks = scales::trans_breaks("log10", function(x) 10^x)  # Major gridlines at log10 integers
  ) +
  facet_grid(rows = vars(Environment), cols = vars(ERM), scales = "free") +
  theme_bw(base_size = 16) +
  theme(legend.position = "none",
        axis.text.x = element_text(size = 12, angle = 45, margin = margin(t = 12)))

getwd()

ggsave("../../output/Manuscript_Figs/all_thresholds_violin_sobol.jpg",
...
# Calculate the 99th percentile for each group to identify outliers
outliers_mass <- all_thresholds_sobol_long_filtered %>%
...
# Ggplot of Monte Carlo SSD thresholds
all_thresholds_violin_sobol_mass <- all_thresholds_sobol_long_filtered %>% 
  filter(grepl("ug.L", Metric)) %>%
  mutate(ERM = case_when(
    grepl("Tissue", Metric, ignore.case = T) ~ "Tissue Translocation",
    grepl("Food", Metric, ignore.case = T) ~ "Food Dilution"
...
ggsave("../../output/Manuscript_Figs/all_thresholds_violin_sobol_mass.jpg",
...
MC_results_tox_sobol <- sobol_results_filtered[1:nrow(mat_filtered)] #sobol_results[1:nrow(mat_filtered)]
...
simple_RSD <- summary_stats_base_thresholds_sobol %>% 
  #mutate(across(where(is.numeric), ~scales::comma(.))) %>% 
  mutate(percentRSD = paste0( signif( (100* Std_Dev) /  Mean, 3), " %"  ))%>% 
  filter(grepl("Default", Metric)) %>% 
...
mat2_tissue_marine <- mat_filtered[, tissue_marine_params] 
...
mat2_tissue_marine_filtered <- mat2_tissue_marine_filtered[, tissue_marine_params] %>% 
  ...
  rename("Length Alpha" = "alpha.marine",
         ...
         "Tissue Trans. Limit (µm)" = "upper.tissue.trans.size.um")
...
scatter_marine_tissue_t3_filtered <- sensobol::plot_scatter(
  ...
  ylab("Tier 3 Threshold (Particles/L)") +
ggtitle("Tissue Translocation: Marine") +
...
# Subset 'mat2' to include only marine parameters
mat2_food_marine <- mat_filtered[, food_marine_params] %>% 
  rename("Length Alpha" = "alpha.marine",
...
# Subset 'mat2' to include only marine parameters
mat2_tissue_freshwater <- mat_filtered[, tissue_freshwater_params] %>% 
...
scatter_freshwater_tissue_t3 <- sensobol::plot_scatter(data = mat2_tissue_freshwater,
                                                   ...
                                                   method = "bin") + 
  ylab("Tier 3 Threshold (Particles/L)") +
                            ggtitle("Tissue Translocation: Freshwater") +
  theme(plot.title =  element_text(size = 15, face = "bold", hjust = 0.5),
...
mat2_food_freshwater <- mat_filtered[, food_freshwater_params] %>% 
  rename("Length Alpha" = "alpha.freshwater",
...
scatterplots <- combined_plot <- ggpubr::ggarrange(
  scatter_marine_food_t3 + ggtitle("Marine") + ylab("Food Dilution") + theme(axis.title.y = element_text(size = 25, face = "bold"),
                                                                             plot.title = element_text(size = 25, face = "bold")), 
  scatter_freshwater_food_t3 + ggtitle("Freshwater") + theme(plot.title = element_text(size = 25, face = "bold")),
  scatter_marine_tissue_t3_filtered + ylab("Tissue Translocation") + theme(axis.title.y = element_text(size = 25, face = "bold"),
                                                                           plot.title = element_blank()) ,
  scatter_freshwater_tissue_t3 + theme(plot.title = element_blank()),
...
ggsave(filename = "sobol_scatterplots.jpg",
       dpi = 300,
       path = "../../output/Manuscript_Figs/", 
       plot = scatterplots, width = 11.8, height = 8, units = "in")
...
```{r}
multi_food <- plot_multiscatter(data = mat_filtered, 
                  N = n_row_sobol,
                  Y = Y_marine_food_T3, 
...
```{r}
multi_tissue <- plot_multiscatter(data = mat_filtered, 
                                N = n_row_sobol,
                                Y = Y_freshwater_tissue_T3, 
...
mat_valid_marine_food <- mat_filtered[valid_indices_marine_food, ]
mat_valid_marine_tissue <- mat_filtered[valid_indices_marine_tissue, ]
mat_valid_freshwater_food <- mat_filtered[valid_indices_freshwater_food, ]
mat_valid_freshwater_tissue <- mat_filtered[valid_indices_freshwater_tissue, ]
...
n_row_sobol_marine_food <- as.integer(N_valid_marine_food / ncol(mat_filtered))
n_row_sobol_marine_tissue <- as.integer(N_valid_marine_tissue / ncol(mat_filtered))
n_row_sobol_freshwater_food <- as.integer(N_valid_freshwater_food / ncol(mat_filtered))
n_row_sobol_freshwater_tissue <- as.integer(N_valid_freshwater_tissue / ncol(mat_filtered))

# Calculate the length_y_rows for each outcome
length_y_rows_marine_food <- n_row_sobol_marine_food * ncol(mat_filtered)
length_y_rows_marine_tissue <- n_row_sobol_marine_tissue * ncol(mat_filtered)
length_y_rows_freshwater_food <- n_row_sobol_freshwater_food * ncol(mat_filtered)
length_y_rows_freshwater_tissue <- n_row_sobol_freshwater_tissue * ncol(mat_filtered)
...
```{r}
cat(crayon::green("MC-Sim Script finished!"),
    "Now make sure you run the ToMEx2_EcoTox.Rmd script now to reproduce results in the manuscript.")
```

```{r eval=FALSE, include=FALSE}

mat_valid <- mat_filtered[valid_indices, ]
...
```{r eval=FALSE, include=FALSE}
R <- n_sobol/3# 4000 #bootstrap iterations
...
clean_and_label_results <- function(sobol_results_filtered, environment, context) {
  ...
  sobol_results_filtered$results %>%
    mutate(
    ...
    mutate(parameter_label = case_when(grepl("alpha", parameters) ~ "Length Alpha",
                                       ...
                                       grepl("tissue", parameters) ~ "Tissue Trans. Limit (µm)")) 
}
