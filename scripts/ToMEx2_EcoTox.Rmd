...
base_thresholds_df <- rbind(base_tissue_marine, base_food_marine, base_tissue_freshwater, base_food_freshwater) %>% 
...
# #example usage
 #tier1_2_food_matrices <- prep_data(results_df_food_t3_t4_marine)
```
...
```{r}
prepare_plot_data <- function(pSSD, data_matrices, species_data_source, quantile_type = 7) {
...
pSSD_plot <- function(quantiles_df, species_data, data_name, environment_name, tier_name, 
  ...
  x_limits <- c(
    min(c(quantiles_df$iv, species_data$NOEC_log10), na.rm = TRUE) + 2,  # Minimum value from data + 2 orders of magnitude
    max_species_value + 3  # Add 3 orders of magnitude to the maximum species value
  )
  ...
  ggplot() +
    ...
    geom_label_repel(
      ...
      size = 3.5,  # Increase label size
      fill = "white",  # Add a white background to labels
      alpha = 0.8,
      box.padding = 0.8,  # Add padding around the label box
      point.padding = 0.8,  # Add padding between points and labels
      segment.alpha = 0.7,  # Make connecting lines semi-transparent
      max.overlaps = Inf,  # Allow all labels to be displayed
      max.time = 10,  # Allow up to 5 seconds to resolve overlaps
      max.iter = 10000,  # Allow up to 10,000 iterations
      force = 5 #increase repulsion between labels
    ) +
...
base_thresholds_df <- rbind(base_tissue, base_food) %>% 
