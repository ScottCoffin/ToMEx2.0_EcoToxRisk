...
This markdown script produces (nearly) all of the figures, tables, and data in the Coffin et al. (2025) publication. Before running this script, ensure that the 'scripts/monte carlo/EcoTox_MonteCarlo.Rmd' script has first been run. That script produces the monte-carlo aligned dataset, which is used in the PSSD++ code below. I have separated these two scripts due to the massive amount of time that the other script takes to run. Due to the size of the dataset produced (>100 MB), it is not feasible to upload that to Github, so the user will need to run this locally  if they would like to recreate the analysis in the publication. 
...
#check SSDtools package version is 0.3.7 (and reinstall if necessary)
source("utils/check_pckg.R")
...
theme.dark <- theme_minimal(base_size = 15) +
...
source("utils/alignment_functions.R")
```
...
aoc_z <- readRDS("../data/input/aoc_z_tomex2.RDS") %>%  #dataset is prepped in shiny repo
  ...
  # Leah was testing how the thresholds would change if we removed these two key studies. Leave this commented out though.
  #filter(!doi %in% c("10.1016/j.marpolbul.2021.112369", "10.3390/nano11030649")) %>%  #filter out Capolupo et al. (2021) and Richardson et al. (2021)
...
#write to csv
write.csv(aoc_aligned_no_filters,
          "../data/output/aoc_aligned_no_filters.csv",
          row.names = FALSE)
...
SSD_function_t1_v1 <- SSD_function_t1
SSD_function_t2_v1 <- SSD_function_t2
SSD_function_t3_4_v1 <- SSD_function_t3_4
# save alt versions for call below (bc we use slightly different outputs for MC analysis, which have the same names)
```
...
# get thresholds
small.default.t1 <- SSD_function_t1_v1(filtered.data = filtered.data.small.default_t1.2, hcxlcl = 5)
small.default.t2 <- SSD_function_t2_v1(filtered.data = filtered.data.small.default_t1.2, hcx = 5)
small.default.t3 <- SSD_function_t3_4_v1(filtered.data = filtered.data.small.default_t3.4, hcx = 5)
small.default.t4 <- SSD_function_t3_4_v1(filtered.data = filtered.data.small.default_t3.4, hcx = 10)
...
# get thresholds
large.default.t1 <- SSD_function_t1_v1(filtered.data = filtered.data.large.default_t1.2, hcxlcl = 5)
large.default.t2 <- SSD_function_t2_v1(filtered.data = filtered.data.large.default_t1.2, hcx = 5)
large.default.t3 <- SSD_function_t3_4_v1(filtered.data = filtered.data.large.default_t3.4, hcx = 5)
large.default.t4 <- SSD_function_t3_4_v1(filtered.data = filtered.data.large.default_t3.4, hcx = 10)
...
## apply function
marine_alpha_thresholds <- derive_thresholds(aligned_data = aoc_marine_alphas_aligned)

saveRDS(marine_alpha_thresholds, "../data/output/marine_alpha_thresholds.rds")
...
saveRDS(aoc_aligned, "../output/data/aligned_data.rds")
...
##### MARINE THRESHOLDS #####
```{r}
formatted_marine_base <- marine_only_thresholds$base_thresholds %>% 
  mutate_if(is.numeric, ~signif(., 3)) %>% 
  ...
  select(Tier, "Food (Marine)", "Tissue (Marine)") %>% 
    filter(Tier != "Tier1") #Tier 1 is just the 95% CI of Tier2, and Tier1 is printing incorrectly

formatted_marine_base
```


##### FRESHWATER THRESHOLDS #####
```{r}
formatted_freshwater_base <- freshwater_only_thresholds$base_thresholds %>% 
  mutate_if(is.numeric, ~signif(., 3)) %>% 
  #mutate(across(where(is.numeric), ~scales::label_comma(.))) %>% 
  mutate("Food (Freshwater)" = paste0(`Food Dilution (Default)`, 
                                      " (", `Food Dilution (5th %)`, " to ", `Food Dilution (95th %)`,")"),
         "Tissue (Freshwater)" = paste0(`Tissue Translocation (Default)`, 
                                        " (", `Tissue Translocation (5th %)`, " to ", `Tissue Translocation (95th %)`,")")) %>% 
  select(Tier, "Food (Freshwater)", "Tissue (Freshwater)") %>% 
  filter(Tier != "Tier1") #Tier 1 is just the 95% CI of Tier2, and Tier1 is printing incorrectly

formatted_freshwater_base
```


```{r}
getwd()
formatted_thresholds <- bind_cols(formatted_marine_base, formatted_freshwater_base)
write.csv(formatted_thresholds, "../data/output/formatted_thresholds.csv")
```
...
## Summary Statistics
First annotate key data regarding whether it is used in threshold derivation or not
```{r}
## how many studies/data points/species exist in starting and final databases?
aoc_z_summary_start <- aoc_z %>%
  mutate(poly_f = case_when(
    poly_f == "Mix - See Original Study" ~ "Mixture",
    poly_f == "Not Reported" ~ "Polymer Not Reported",
    T ~ poly_f
  )) %>% 
  mutate(size_f = case_when(
    is.na(size_f) ~ "Size Not Reported",
    size_f == "Not Reported" ~ "Size Not Reported",
    T ~ size_f
  )) %>% 
  mutate(shape_f = case_when(
    shape_f == "Not Reported" ~ "Shape Not Reported",
    T ~ shape_f
  )) %>% 
  mutate(Speices = case_when(
    is.na(Species) ~ "Species Not Reported",
    T ~ Species
  )) %>% 
  droplevels() %>% 
  # Add a column to indicate if data is available based on polydispersity conditions
  mutate(
    data_available = case_when(
      polydispersity == "polydisperse" & 
        !is.na(particle.surface.area.um2.min) & 
        !is.na(particle.surface.area.um2.max) ~ "Available",
      polydispersity == "monodisperse" & 
        !is.na(particle.surface.area.um2) ~ "Available",
        #Remove 26C temperature treatment data from Jaimukar et al. 2018
      article == 42 & media.temp == 26 ~ "Not Available", 
      TRUE ~ "Not Available"
    )
  ) %>%
  # Add thresholds1_2 column based on specific criteria
  mutate(
    thresholds1_2 = case_when(
      data_available == "Available" &
        !is.na(dose.particles.mL.master) &
        !environment %in% c("Terrestrial", "Not Reported") &
        !Group %in% c("Bacterium", "Plant") &
        effect.metric != "HONEC" &
        !is.na(effect.metric) &
        exp_type_f == "Particle Only" &
        tier_zero_tech_f == "Red Criteria Passed" &
        ...
        risk.13 != 0 ~ "Meets Criteria",
      TRUE ~ "Does Not Meet Criteria"
    )
  ) %>%
  # Add thresholds3_4 column based on additional criteria
  mutate(
    thresholds3_4 = case_when(
      thresholds1_2 == "Meets Criteria" &
        risk.13 != 1 &
        bio_f %in% c("Organism", "Population") ~ "Meets Criteria",
      TRUE ~ "Does Not Meet Criteria"
    )
  )
```
...
af_summary <- aoc_z_summary_start %>% 
  mutate(af.noec = case_when(
    is.na(af.noec) ~ "None",
    T ~ as.character(af.noec)
    ),
    af.time = case_when(
      is.na(af.time) ~ "None",
      T ~ as.character(af.time)
    )
  ) %>% 
  mutate(AF.total = case_when(
    is.na(AF.total) ~ "UF: Intermediary exposure concentration",
    T ~ paste0("UF:", as.character(AF.total)))) %>% 
  group_by(environment, AF.total
           #, af.noec, af.time
           ) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = "environment", values_from = n) %>% 
  arrange(desc(Marine)) %>% 
  mutate(Freshwater = as.character(Freshwater),
         Marine = as.character(Marine)) %>% 
  column_to_rownames("AF.total")

# effect metrics breakdown
effect_metrics <- aoc_z_summary_start %>% 
  group_by(environment, effect.metric) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = "environment", values_from = n) %>% 
  arrange(desc(Marine)) %>% 
  mutate(Freshwater = as.character(Freshwater),
  ...
  mutate(effect.metric = case_when(
    is.na(effect.metric) ~ "Intermediary exposure concentration",
    T ~ effect.metric
  )) %>% 
  column_to_rownames("effect.metric")

# particle shape breakdown
particle_shapes <- aoc_z_summary_start %>% 
  group_by(environment, shape_f) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = "environment", values_from = n) %>% 
  arrange(desc(Marine)) %>% 
  mutate(Freshwater = as.character(Freshwater),
         Marine = as.character(Marine)) %>% 
  column_to_rownames("shape_f")

# polymer breakdown
polymer_breakdown <- aoc_z_summary_start %>% 
  group_by(environment, poly_f) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = "environment", values_from = n) %>% 
  arrange(desc(Marine)) %>% 
  mutate(Freshwater = as.character(Freshwater),
         Marine = as.character(Marine)) %>% 
  column_to_rownames("poly_f")

# Define the custom order for size bins
size_order <- c("1nm < 100nm", "100nm < 1µm", "1µm < 100µm", "100µm < 1mm", "1mm < 5mm", "Not Reported")

# size breakdown
size_breakdown <- aoc_z_summary_start %>% 
  group_by(environment, size_f) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = "environment", values_from = n) %>% 
  arrange(factor(size_f, levels = size_order)) %>%  # Arrange by custom order
  mutate(Freshwater = as.character(Freshwater),
         Marine = as.character(Marine)) %>% 
  column_to_rownames("size_f")

# species breakdown
group_breakdown <- aoc_z_summary_start %>% 
  group_by(environment, Group) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = "environment", values_from = n) %>% 
  arrange(desc(Marine)) %>%  # Arrange by custom order
  mutate(Freshwater = as.character(Freshwater),
         Marine = as.character(Marine)) %>% 
  column_to_rownames("Group")

# Taxa breakdown
taxa_summary <- aoc_z_summary_start %>% 
  mutate(taxa = as.factor(stringr::str_extract(as.character(Species), "\\S+"))) %>% 
  mutate(taxa = case_when(
    is.na(taxa) ~ "Not reported",
    T ~ taxa
  )) %>% 
  group_by(environment, taxa) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = c("environment"), values_from = n) %>% 
 arrange(desc(Marine)) %>%  # Arrange by custom order
  mutate(Freshwater = as.character(Freshwater),
         Marine = as.character(Marine)) %>% 
  column_to_rownames("taxa")

all_studies_summary <- aoc_z_summary_start %>% 
  group_by(environment) %>%
  # Summarize the data and calculate chronic percentage
  summarise(
       n_datapoint = n(),
    n_studies = n_distinct(doi),
 n_species = n_distinct(Species),
    # Calculate percentage of chronic studies
    chronic_percentage = paste0(
      signif(
        sum(acute.chronic_f == "Chronic") / n() * 100, 2
        ),
      "%"),
       # calculate percentage of polydisperse studies
    poly_percentage = paste0(
      signif(
        sum(polydispersity == "polydisperse") / n() * 100, 2
        ),
      "%"),
    .groups = "drop" # Avoid unnecessary grouping in the final output
  ) %>%
  rename("Environment" = environment,
         "Number of Species" = n_species,
         "Number of Studies" = n_studies,
         "Number of Data Points" = n_datapoint,
         "% Chronic" = chronic_percentage,
         "% Polydisperse" = poly_percentage
         ) %>% 
  t() %>% 
  as.data.frame() %>% 
  rename("Freshwater" = V1,
         "Marine" = V2) %>% 
  dplyr::slice(-1) %>% 
  bind_rows(effect_metrics,
            af_summary,
            group_breakdown,
            taxa_summary,
            size_breakdown,
...
#write.csv(all_studies_summary, "../data/output/all_studies_summary.csv")

all_studies_summary
```


#### Just studies used in Thresholds
```{r}
# get just the data that passed through all filters

tiers1_2_used <- bind_rows(marine_alpha_thresholds$data$filtered.data.small.default_t1.2,
                           marine_alpha_thresholds$data$filtered.data.large.default_t1.2,
                           freshwater_only_thresholds$data$filtered.data.small.default_t1.2,
                           freshwater_only_thresholds$data$filtered.data.large.default_t1.2
                           ) %>%
  mutate(thresholds1_2 = "Meets Criteria") %>% 
  distinct(rowid, .keep_all = T)

tiers3_4_used <- bind_rows(freshwater_only_thresholds$data$filtered.data.small.default_t3.4,
                           marine_alpha_thresholds$data$filtered.data.small.default_t3.4,
                           freshwater_only_thresholds$data$filtered.data.large.default_t3.4,
                           marine_alpha_thresholds$data$filtered.data.large.default_t3.4,
                           ) %>% 
  mutate(thresholds3_4 = "Meets Criteria") %>% 
  distinct(rowid, .keep_all = T)
```

```{r}
############### TIERS 1/2 ############
# Group by threshold, status, and environment
data_summary_t1_2 <- tiers1_2_used %>% 
   # Reshape data to long format for thresholds
  pivot_longer(
    cols = c(thresholds1_2),
    names_to = "threshold",
    values_to = "status"
  ) %>% 
   filter(status == "Meets Criteria") %>% 
  group_by(threshold, environment) %>%
  # Summarize the data and calculate chronic percentage
  summarise(
     n_datapoint = n(),
    n_studies = n_distinct(doi),
  ...
  t() %>% 
  as.data.frame() %>% 
  rename("Freshwater (Tier 1/2)" = V1,
         "Marine (Tier 1/2)" = V2
         ) %>% 
  dplyr::slice(-c(1,2))

# Assessment Factors breakdown
af_summary_used_t1_2 <- tiers1_2_used %>% 
   pivot_longer(
    cols = c(thresholds1_2),
    names_to = "threshold",
    values_to = "status"
  ...
  mutate(AF.total = case_when(
    is.na(AF.total) ~ "UF: Intermediary exposure concentration",
    T ~ paste0("UF:", as.character(AF.total)))) %>% 
  group_by(threshold, environment, AF.total
           #, af.noec, af.time
           ) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = c("environment", "threshold"), values_from = n) %>% 
  arrange(desc(Marine_thresholds1_2)) %>% 
  mutate("Freshwater (Tier 1/2)" = as.character(Freshwater_thresholds1_2),
         "Marine (Tier 1/2)" = as.character(Marine_thresholds1_2)
       ) %>% 
  select(-c(Freshwater_thresholds1_2,
            Marine_thresholds1_2
            )) %>% 
  column_to_rownames("AF.total")

effect_metrics_used_t1_2 <- tiers1_2_used %>% 
   pivot_longer(
    cols = c(thresholds1_2),
    names_to = "threshold",
    values_to = "status"
   ...
   filter(status == "Meets Criteria") %>% 
  group_by(threshold, environment, effect.metric) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = c("environment", "threshold"), values_from = n) %>% 
  arrange(desc(Marine_thresholds1_2)) %>% 
  mutate("Freshwater (Tier 1/2)" = as.character(Freshwater_thresholds1_2),
...
# particle shape breakdown
particle_shapes_used_t1_2 <- tiers1_2_used %>% 
    pivot_longer(
    cols = c(thresholds1_2),
    names_to = "threshold",
    values_to = "status"
   ...
   filter(status == "Meets Criteria") %>% 
  group_by(threshold, environment, shape_f) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = c("environment", "threshold"), values_from = n) %>% 
  arrange(desc(Marine_thresholds1_2)) %>% 
  mutate("Freshwater (Tier 1/2)" = as.character(Freshwater_thresholds1_2),
...
# polymer breakdown
polymer_breakdown_used_t1_2 <- tiers1_2_used %>% 
    pivot_longer(
    cols = c(thresholds1_2),
    names_to = "threshold",
    values_to = "status"
   ...
   filter(status == "Meets Criteria") %>% 
  group_by(threshold, environment, poly_f) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = c("environment", "threshold"), values_from = n) %>% 
  arrange(desc(Marine_thresholds1_2)) %>% 
  mutate("Freshwater (Tier 1/2)" = as.character(Freshwater_thresholds1_2),
         "Marine (Tier 1/2)" = as.character(Marine_thresholds1_2)) %>% 
  select(-c(Freshwater_thresholds1_2, Marine_thresholds1_2)) %>% 
  column_to_rownames("poly_f")

# size breakdown
size_breakdown_used_t1_2 <- tiers1_2_used %>% 
   pivot_longer(
    cols = c(thresholds1_2),
    names_to = "threshold",
    values_to = "status"
   ...
   filter(status == "Meets Criteria") %>% 
  group_by(threshold, environment, size_f) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = c("environment", "threshold"), values_from = n) %>% 
  arrange(factor(size_f, levels = size_order)) %>%  # Arrange by custom order
  mutate("Freshwater (Tier 1/2)" = as.character(Freshwater_thresholds1_2),
...
# group breakdown
group_breakdown_used_t1_2 <- tiers1_2_used %>% 
    pivot_longer(
    cols = c(thresholds1_2),
    names_to = "threshold",
    values_to = "status"
   ...
   filter(status == "Meets Criteria") %>% 
  group_by(threshold, environment, Group) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = c("environment", "threshold"), values_from = n) %>% 
  arrange(desc(Marine_thresholds1_2)) %>% 
  mutate("Freshwater (Tier 1/2)" = as.character(Freshwater_thresholds1_2),
...
# Taxa breakdown
taxa_summary_used_t1_2 <- tiers1_2_used %>% 
   pivot_longer(
    cols = c(thresholds1_2),
    names_to = "threshold",
    values_to = "status"
   ...
   filter(status == "Meets Criteria") %>% 
  mutate(taxa = as.factor(stringr::str_extract(as.character(Species), "\\S+"))) %>% 
  group_by(threshold, environment, taxa
           #, af.noec, af.time
           ) %>% 
  summarize(n = n()) %>% 
   pivot_wider(names_from = c("environment", "threshold"), values_from = n) %>% 
...
############### TIERS 3/4 ############
# Group by threshold, status, and environment
data_summary_t3_4 <- tiers3_4_used %>% 
   # Reshape data to long format for thresholds
  pivot_longer(
    cols = c(thresholds3_4),
    names_to = "threshold",
    values_to = "status"
  ) %>% 
  group_by(threshold, environment) %>%
  ...
  t() %>% 
  as.data.frame() %>% 
  rename("Freshwater (Tier 3/4)" = V1,
         "Marine (Tier 3/4)" = V2
         ) %>% 
  dplyr::slice(-c(1,2))

# Assessment Factors breakdown
af_summary_used_t3_4 <- tiers3_4_used %>% 
   pivot_longer(
    cols = c(thresholds3_4),
    names_to = "threshold",
    values_to = "status"
  ...
  summarize(n = n()) %>% 
   pivot_wider(names_from = c("environment", "threshold"), values_from = n) %>% 
  arrange(desc(Marine_thresholds3_4)) %>% 
  mutate("Freshwater (Tier 3/4)" = as.character(Freshwater_thresholds3_4),
          "Marine (Tier 3/4)" = as.character(Marine_thresholds3_4)
       ) %>% 
  select(-c(Freshwater_thresholds3_4,# Freshwater_thresholds3_4,
            Marine_thresholds3_4#, Marine_thresholds3_4
            )) %>% 
  column_to_rownames("AF.total")

effect_metrics_used_t3_4 <- tiers3_4_used %>% 
   pivot_longer(
    cols = c(thresholds3_4),
    names_to = "threshold",
    values_to = "status"
...
# particle shape breakdown
particle_shapes_used_t3_4 <- tiers3_4_used %>% 
    pivot_longer(
    cols = c(thresholds3_4),
    names_to = "threshold",
    values_to = "status"
  ...
  select(-c(Freshwater_thresholds3_4,
            Marine_thresholds3_4
            )) %>% 
  column_to_rownames("shape_f")

# polymer breakdown
polymer_breakdown_used_t3_4 <- tiers3_4_used %>% 
    pivot_longer(
    cols = c(thresholds3_4, thresholds3_4),
    names_to = "threshold",
    values_to = "status"
  ) %>% 
   filter(status == "Meets Criteria") %>% 
  ...
  mutate("Freshwater (Tier 3/4)" = as.character(Freshwater_thresholds3_4),
         "Marine (Tier 3/4)" = as.character(Marine_thresholds3_4)) %>% 
  select(-c(Freshwater_thresholds3_4, Marine_thresholds3_4)) %>% 
  column_to_rownames("poly_f")

# size breakdown
size_breakdown_used_t3_4 <- tiers3_4_used %>% 
   pivot_longer(
    cols = c(thresholds3_4),
    names_to = "threshold",
    values_to = "status"
...
# group breakdown
group_breakdown_used_t3_4 <- tiers3_4_used %>% 
    pivot_longer(
    cols = c(thresholds3_4),
    names_to = "threshold",
    values_to = "status"
  ...
  column_to_rownames("Group")
# Taxa breakdown
taxa_summary_used_t3_4 <- tiers3_4_used %>% 
   pivot_longer(
    cols = c(thresholds3_4),
    names_to = "threshold",
    values_to = "status"
...
data_used_thresholds_summary_t1_2 <- bind_rows(data_summary_t1_2,
                                          effect_metrics_used_t1_2, 
                                          af_summary_used_t1_2,
                                          group_breakdown_used_t1_2,
                                          taxa_summary_used_t1_2,
                                          size_breakdown_used_t1_2,
                                          particle_shapes_used_t1_2,
                                          polymer_breakdown_used_t1_2) %>% 
  rownames_to_column("Factor") 

data_used_thresholds_summary_t3_4 <- bind_rows(data_summary_t3_4,
                                          effect_metrics_used_t3_4, 
                                          af_summary_used_t3_4,
                                          group_breakdown_used_t3_4,
                                          taxa_summary_used_t3_4,
                                          size_breakdown_used_t3_4,
                                          particle_shapes_used_t3_4,
                                          polymer_breakdown_used_t3_4) %>% 
  rownames_to_column("Factor") 


data_used_thresholds_summary <- left_join(data_used_thresholds_summary_t1_2,
                                          data_used_thresholds_summary_t3_4,
                                          by = "Factor") %>% 
  relocate("Freshwater (Tier 3/4)", .after = "Freshwater (Tier 1/2)") %>% 
  mutate_all(~replace(., is.na(.), 0))



data_used_thresholds_summary
```
Both combined
```{r}
full_summary <- left_join(all_studies_summary,
                          data_used_thresholds_summary,
          by = "Factor") %>% 
  mutate_all(~replace(., is.na(.), 0)) %>% 
  relocate("Marine", .before = "Marine (Tier 1/2)")

write.csv(full_summary, "../data/output/full_database_summary.csv")
...
source("utils/ssd_functions.R")
source("utils/RDAmaker_functions.R") #get ToMEx1.0 fxn
source("utils/ToMEx2.0_Data_Tidying_functions.R")
```
...
```
#### Fold-change table
Below we calculate the fold-change of thresholds for marine/freshwater with and without quality filters
```{r}
#all_thresholds_combined <- read.csv("../output/data/all_thresholds_combined_quality.csv")

tissue_fold_change <- all_thresholds_combined %>% 
  filter(label %in% c("risk and tech", "no quality filters"),
         Environment %in% c("Marine", "Freshwater")) %>% 
  select(`Tissue Translocation (Default)`, Tier, label, Environment) %>% 
  pivot_wider(#cols = Tissue.Translocation..Default.,
              names_from = label,
              values_from = `Tissue Translocation (Default)`) %>% 
  mutate(tissue_fold_change = `no quality filters` / `risk and tech`) %>% 
  select(Tier, Environment, tissue_fold_change)

food_fold_change <- all_thresholds_combined %>% 
  filter(label %in% c("risk and tech", "no quality filters"),
         Environment %in% c("Marine", "Freshwater")) %>% 
  select(`Food Dilution (Default)`, Tier, label, Environment) %>% 
  pivot_wider(#cols = Tissue.Translocation..Default.,
              names_from = label,
              values_from = `Food Dilution (Default)`) %>% 
  mutate(food_fold_change = `no quality filters` / `risk and tech`) %>% 
  select(Tier, Environment, food_fold_change)

quality_fold_change <- left_join(food_fold_change, tissue_fold_change) %>% 
  mutate_if(is.numeric, ~signif(.,2))

write.csv(quality_fold_change, "../output/data/quality_fold_change.csv")
...
### Freshwater species with marine params

```{r message=FALSE, warning=FALSE}
# align all data to marine params (done above)
# aoc_marine_alphas_aligned

# derive thresholds using freshwater species but with marine params
freshwater_species_marine_params_thresholds <- derive_thresholds(aoc_marine_alphas_aligned %>% 
                                                                   filter(environment == "Freshwater"))

# compare freshwater species marine params thresholds with freshwater species freshwater params thresholds

formatted_freshwater_species_marine_params_thresholds <- freshwater_species_marine_params_thresholds$base_thresholds %>% 
  mutate_if(is.numeric, ~signif(., 2)) %>% 
  mutate(across(where(is.numeric), ~scales::comma(.))) %>% 
  mutate("Food (Freshwater Sp., marine params)" = paste0(`Food Dilution (Default)`, " (", `Food Dilution (5th %)`, " to ", `Food Dilution (95th %)`,")"),
         "Tissue (Freshwater Sp., marine params)" = paste0(`Tissue Translocation (Default)`, " (", `Tissue Translocation (5th %)`, " to ", `Tissue Translocation (95th %)`,")")) %>% 
  select(Tier, "Food (Freshwater Sp., marine params)", "Tissue (Freshwater Sp., marine params)") 

# rename cols in above calcualted
formatted_freshwater_species_freshwater_params_thresholds <- formatted_freshwater_base %>% 
  rename("Food (Freshwater sp., freshwater params)" = "Food (Freshwater)",
         "Tissue (Freshwater sp., freshwater params)" = "Tissue (Freshwater)")

#join up
params_species_sensitivity_analysis_freshwater_sp <- left_join(formatted_freshwater_species_marine_params_thresholds,
                                                               formatted_freshwater_species_freshwater_params_thresholds)

params_species_sensitivity_analysis_freshwater_sp
```

### Marine species with freshwater params

```{r message=FALSE, warning=FALSE}
# align all data to freshwater params
aoc_freshwater_alphas_aligned <- align_data(aoc_risk_paper,
                
                       R.ave.marine_input = 0.67, # average length to width ratio of microplastics in marine environment (Kooi et al. 2021)
                       H_W_ratio.marine_input = 0.67, # H:W ratio assumed same as width:length ratio (Kooi et al. 2021)
                     R.ave.sediment.marine_input = 0.70, # average length to width ratio of microplastics in marine environment (Kooi et al. 2021)
                       p.ave.marine_input = 1.04, #average density in marine surface water
                       alpha.marine_input = 2.64, #table s4 for marine surface water. length
                       a.sa.marine_input = 2.00, #marine surface area power law
                       a.v.marine_input = 1.68, #a_V for marine surface water volume
                       a.m.marine_input = 1.65, # upper limit fora_m for mass for marine surface water in table S4 
                       a.ssa.marine_input = 2.71, # A_SSA for marine surface water
                                            )

# derive thresholds using freshwater species but with marine params
marine_species_freshwater_params_thresholds <- derive_thresholds(aoc_freshwater_alphas_aligned %>% 
                                                                   filter(environment == "Marine"))

# compare marine species freshwater params thresholds with marine species marine params thresholds

formatted_marine_species_freshwater_params_thresholds <- marine_species_freshwater_params_thresholds$base_thresholds %>% 
  mutate_if(is.numeric, ~signif(., 2)) %>% 
  mutate(across(where(is.numeric), ~scales::comma(.))) %>% 
  mutate("Food (Marine Sp., freshwater params)" = paste0(`Food Dilution (Default)`, " (", `Food Dilution (5th %)`, " to ", `Food Dilution (95th %)`,")"),
         "Tissue (Marine Sp., freshwater params)" = paste0(`Tissue Translocation (Default)`, " (", `Tissue Translocation (5th %)`, " to ", `Tissue Translocation (95th %)`,")")) %>% 
  select(Tier, "Food (Marine Sp., freshwater params)", "Tissue (Marine Sp., freshwater params)") 

# rename cols in above calcualted
formatted_marine_species_marine_params_thresholds <- formatted_marine_base %>% 
  rename("Food (marine sp., marine params)" = "Food (Marine)",
         "Tissue (marine sp., marine params)" = "Tissue (Marine)")

#join up
params_species_sensitivity_analysis_marine_sp <- left_join(formatted_marine_species_freshwater_params_thresholds,
                                                               formatted_marine_species_marine_params_thresholds)

params_species_sensitivity_analysis_marine_sp
```
```{r}
freshwater_marine_params_species_sensitivity_analysis <- 
  left_join(params_species_sensitivity_analysis_freshwater_sp, params_species_sensitivity_analysis_marine_sp)
```
...
tier_colors <- setNames(c("#006f3c", "#b0a704", "#EB8907", "#f0514b"),
...
scale_x_continuous(
  ...
  breaks = c(0.01, 1, 2, 4, 8, 16, 32, 64, 128),
  limits = c(0.01, 130),  # slightly higher than 128 to ensure it appears
  labels = function(x) {
    sapply(x, function(v) {
      if (is.na(v)) {
        NA
      } else if (isTRUE(all.equal(v, 0.01))) {
        "0.01"
      } else {
...
MC performed in scripts/monte carlo/EcoTox_MonteCarlo.Rmd
```{r}
#regular MC
#MC_results <- readRDS("monte carlo/output/MC_results.rds")
#n_sim <- readRDS("monte carlo/output/n_sim.rds")
#n_sim

#sobol sequence generated data is used because the Latin Hypercube Sampling approach more robustly propagates uncertainties than the standard MC approach
#MC_results <- readRDS("monte carlo/output/MC_results_sobol.rds")

MC_results <- readRDS("monte carlo/output/sobol_results_filtered.rds")
#summary_stats_base_thresholds_MC <- readRDS("monte carlo/output/summary_stats_base_thresholds.rds")

summary_stats_base_thresholds_MC <- readRDS("monte carlo/output/summary_stats_base_thresholds_sobol.rds")

n_sim <- summary_stats_base_thresholds_MC$N_sim[1]

n_sim
```

#### Pretty table summarizing MC results
###### Particles/L
```{r}
library(scales)

## Freshwater MC
Freshwater_MC <- summary_stats_base_thresholds_MC %>% 
...
# Marine MC
Marine_MC <-summary_stats_base_thresholds_MC %>% 
  filter(str_detect(Metric, "Default"),
         Environment == "Marine") %>% 
  select(Tier, Metric, Median, contains("Percentile")) %>% 
  mutate(Threshold = paste0(comma(Median), " (", comma(Percentile_5th),
...
write.csv(simple_thresholds_MC,
          "../output/data/simple_thresholds_table.csv")
...
```{r}
# Freshwater MC
MC_RSD <- summary_stats_base_thresholds_MC %>% 
  filter(str_detect(Metric, "Default")) %>% 
  select(Tier, Environment, Metric, Mean, Std_Dev) %>% 
  mutate(Metric_Environment = paste(Metric, Environment)) %>% 
  mutate(percent_RSD = paste0(signif(100 * (Std_Dev / Mean),3), "%")) %>% 
  select(-c(Mean, Std_Dev, Environment, Metric)) %>% 
  pivot_wider(names_from = Metric_Environment, values_from = percent_RSD) %>% 
  rename_with(~ gsub("(Default)", "", .x), everything())
...
## Freshwater MC
Freshwater_MC_mass <- summary_stats_base_thresholds_MC %>% 
...
# Marine MC
Marine_MC_mass <- summary_stats_base_thresholds_MC %>% 
  filter(str_detect(Metric, "ug.L"),
         Environment == "Marine") %>% 
  select(Tier, Metric, Median, contains("Percentile")) %>% 
  mutate(Threshold = paste0(comma(Median), " (", comma(Percentile_5th),
...
simple_thresholds_MC_mass <- left_join(Freshwater_MC_mass, Marine_MC_mass,
                                             by = "Tier")

write.csv(simple_thresholds_MC_mass,
          "../output/data/simple_thresholds_MC_mass.csv")

simple_thresholds_MC_mass
```
 
###### Metadata
```{r}
## Freshwater MC
Freshwater_MC_meta <- summary_stats_base_thresholds_MC %>% 
  filter(str_detect(Metric, "N"),
         Environment == "Freshwater") %>% 
  select(Tier, Metric, Median, contains("Percentile")) %>% 
  mutate(Threshold = paste0(comma(Median), " (", comma(Percentile_5th),
                            " to ", comma(Percentile_95th), ")")) %>% 
  select(-c(Median, Percentile_5th, Percentile_95th)) %>% 
  pivot_wider(names_from = Metric, values_from = Threshold) %>% 
  rename_with(~ gsub("N", "Freshwater", .x), everything())

# Marine MC
Marine_MC_meta <-summary_stats_base_thresholds_MC %>% 
 filter(str_detect(Metric, "N"),
         Environment == "Marine") %>% 
  select(Tier, Metric, Median, contains("Percentile")) %>% 
  mutate(Threshold = paste0(comma(Median), " (", comma(Percentile_5th),
  ...
  select(-c(Median, Percentile_5th, Percentile_95th)) %>% 
  pivot_wider(names_from = Metric, values_from = Threshold) %>% 
  rename_with(~ gsub("N", "Marine", .x), everything())

simple_thresholds_MC_meta <- left_join(Freshwater_MC_meta, Marine_MC_meta)

write.csv(simple_thresholds_MC_meta,
          "../output/data/simple_thresholds_table_meta.csv")

simple_thresholds_MC_meta
```
...
```{r}
all_thresholds_long <- readRDS("monte carlo/output/all_thresholds_sobol_long_filtered.rds")
...
all_MC_data_long <- all_thresholds_long %>%
  mutate(ERM = case_when(
    grepl("Tissue", Metric) ~ "Tissue Translocation",
    grepl("Food", Metric) ~ "Food Dilution"
  )) 

prepared_data <-  all_MC_data_long %>%
  filter(
...
#### Comparison of MC and Deterministic Thresholds
```{r}

### MARINE ##
base_tissue_marine <- data.frame(Tier = c("1", "2", "3", "4"),
                          ERM = "Tissue Translocation", 
                          Environment = "Marine",
                          PNEC = marine_only_thresholds$base_thresholds$`Tissue Translocation (Default)`,
                          PNEClcl = marine_only_thresholds$base_thresholds$`Tissue Translocation (5th %)`,
                          PNECucl = marine_only_thresholds$base_thresholds$`Tissue Translocation (95th %)`)

base_food_marine <- data.frame(Tier = c("1", "2", "3", "4"),
                        ERM = "Food Dilution", 
                        Environment = "Marine",
                        PNEC = marine_only_thresholds$base_thresholds$`Food Dilution (Default)`,
                        PNEClcl = marine_only_thresholds$base_thresholds$`Food Dilution (5th %)`,
                        PNECucl = marine_only_thresholds$base_thresholds$`Food Dilution (95th %)`)


#### FRESHWATER ####
base_tissue_freshwater <- data.frame(Tier = c("1", "2", "3", "4"),
                          ERM = "Tissue Translocation", 
                          Environment = "Freshwater",
                          PNEC = freshwater_only_thresholds$base_thresholds$`Tissue Translocation (Default)`,
                          PNEClcl = freshwater_only_thresholds$base_thresholds$`Tissue Translocation (5th %)`,
                          PNECucl = freshwater_only_thresholds$base_thresholds$`Tissue Translocation (95th %)`)

base_food_freshwater <- data.frame(Tier = c("1", "2", "3", "4"),
                        ERM = "Food Dilution", 
                        Environment = "Freshwater",
                        PNEC = freshwater_only_thresholds$base_thresholds$`Food Dilution (Default)`,
                        PNEClcl = freshwater_only_thresholds$base_thresholds$`Food Dilution (5th %)`,
                        PNECucl = freshwater_only_thresholds$base_thresholds$`Food Dilution (95th %)`)

base_thresholds_df <- bind_rows(base_tissue_marine, base_food_marine, base_tissue_freshwater, base_food_freshwater) %>% 
    mutate(y_position = case_when(
    ERM == "Tissue Translocation" & Environment == "Freshwater" ~ 2,  # Offset for Tissue Translocation
    ERM == "Tissue Translocation" & Environment == "Marine" ~ 0.9,  # Offset for Tissue Translocation
    ERM == "Food Dilution" & Environment == "Freshwater" ~ 1.6,  # Offset for Tissue Translocation
    ERM == "Food Dilution" & Environment == "Marine" ~ 0.5,  # Offset for Tissue Translocation
  ))
...
#data prep
t2f_freshwater <- all_MC_data_long  %>% 
  filter(ERM == "Food Dilution",
         Tier == "Tier2",
         Environment == "Freshwater") %>% 
  rename(PNEC = Value) %>% 
...
#data prep
t2t_freshwater <- all_MC_data_long  %>% 
  filter(ERM == "Tissue Translocation",
         Tier == "Tier2",
         Environment == "Freshwater") %>% 
  rename(PNEC = Value) %>% 
...
## join DFs
PNEC_dfs <- rbind(t2f_marine, t2f_freshwater, t2t_marine, t2t_freshwater) %>% 
 mutate(y_position = (as.numeric(factor(Environment)) - 2) + (as.numeric(factor(ERM)) - 1.5) * 0.4)  # Adjust 0.1 or other value as needed

#plot
PNEC_MCSSDvSSD <- ggplot() +
   geom_violin(data = PNEC_dfs, aes(x = PNEC, y = Environment, fill = ERM, color = ERM),
               draw_quantiles = c(0.05, 0.5, 0.95), linewidth = 1.2,
               alpha = 0.75) +
   # geom_jitter(data = PNEC_dfs, aes(x = PNEC, y = y_position, fill = ERM, color = ERM),
   #             height = 0.15, size = 1.5, alpha = 0.02) +  # Jittered points
   geom_point(data = base_thresholds_df %>% filter(Tier == 2), 
   ...
   geom_errorbarh(data = base_thresholds_df %>% filter(Tier == 2), 
 ...
 #  scale_x_log10(breaks = c(0.01, 0.1, 1, 10, 100, 1000, 10000, 1000000, 10000000),
  ...
  labs(title = "Comparison of Monte-Carlo [mcSSDs] and Deterministic [dSSD] Thresholds",
       subtitle = "(Violin Plots = mcSSDs; Diamonds/Error-Bars = dSSD; Tier 2 only)",
...
PNEC_MCSSDvSSD_dark <- PNEC_MCSSDvSSD + theme.dark
PNEC_MCSSDvSSD_light <- PNEC_MCSSDvSSD + theme.light
...
ggsave(filename = "PNEC_pSSDvSSD.jpg",
       path = "../output/presentation_Figs/", 
       plot = PNEC_MCSSDvSSD_dark, width = 12, height = 10, units = "in")

ggsave(filename = "PNEC_MCSSDvSSD.tiff",
       path = "../output/Manuscript_Figs/", 
       plot = PNEC_MCSSDvSSD_light, width = 10, height = 8, units = "in")

ggsave(filename = "PNEC_MCSSDvSSD.jpg",
       path = "../output/Manuscript_Figs/", 
       plot = PNEC_MCSSDvSSD_light, width = 10, height = 8, units = "in")

PNEC_MCSSDvSSD_light
```
...
#data prep
t3f_freshwater <- all_MC_data_long  %>% 
  filter(ERM == "Food Dilution",
         Tier == "Tier3",
         Environment == "Freshwater") %>% 
  rename(PNEC = Value) %>% 
...
#data prep
t3t_freshwater <- all_MC_data_long  %>% 
  filter(ERM == "Tissue Translocation",
         Tier == "Tier3",
         Environment == "Freshwater") %>% 
  rename(PNEC = Value) %>% 
...
## join DFs
PNEC_dfs <- rbind(t3f_marine, t3f_freshwater, t3t_marine, t3t_freshwater) %>% 
 mutate(y_position = (as.numeric(factor(Environment)) - 2) + (as.numeric(factor(ERM)) - 1.5) * 0.4)  # Adjust 0.1 or other value as needed
...
PNEC_MCSSDvSSD <- ggplot() +
   ...
   geom_point(data = base_thresholds_df %>% filter(Tier == 3), 
              aes(x = PNEC, y = y_position, fill = ERM),
              size = 5, shape = 23, 
              color = "white",
              alpha = 0.95,
   ...
   geom_errorbarh(data = base_thresholds_df %>% filter(Tier == 3), 
                  aes(xmin = PNEClcl, xmax = PNECucl, y = y_position, fill = ERM, color = ERM),
                  height = 0.2,
                  alpha = 0.75,
                  linewidth = 1.2) +
 ...
 #  scale_x_log10(breaks = c(0.01, 0.1, 1, 10, 100, 1000, 10000, 1000000, 10000000),
  ...
  #               labels = scales::label_comma()) +
  labs(title = "Comparison of Monte-Carlo [mcSSDs] and Deterministic [dSSD] Thresholds",
       subtitle = "(Violin Plots = mcSSDs; Diamonds/Error-Bars = dSSD; Tier 3 only)",
       fill = "Ecologically-Relevant Metric",
       color = "Ecologically-Relevant Metric"
       ) +
  xlab("PNEC (Particles/L; 1 to 5,000 um)") +
...
PNEC_MCSSDvSSD_dark <- PNEC_MCSSDvSSD + theme.dark
PNEC_MCSSDvSSD_light_t3 <- PNEC_MCSSDvSSD + theme.light
...
ggsave(filename = "PNEC_MCSSDvSSD_t3.tiff",
       path = "../output/Manuscript_Figs/", 
       plot = PNEC_MCSSDvSSD_light_t3, width = 10, height = 8, units = "in")

ggsave(filename = "PNEC_MCSSDvSSD_t3.jpg",
       path = "../output/Manuscript_Figs/", 
       plot = PNEC_MCSSDvSSD_light_t3, width = 10, height = 8, units = "in")

PNEC_MCSSDvSSD_light_t3
```
```{r}
summary_PNEC_figure <- ggarrange(PNEC_MCSSDvSSD_light + labs(subtitle = "", title = "Tier 2") + 
                                   theme_minimal(base_size = 13) +
                                   theme(plot.subtitle =  element_blank(),  plot.title = element_text(hjust = 0.5)),
          PNEC_MCSSDvSSD_light_t3 + labs(subtitle = "", title = "Tier 3",) + theme_minimal(base_size = 13) +
            theme(plot.subtitle = element_blank(), plot.title = element_text(hjust = 0.5),
                                                                                  axis.text.y = element_blank(),
                                                                                  axis.title.y = element_blank()),
          nrow = 1,
...
ggsave(filename = "summary_PNEC_figure.jpg",
       dpi = 400,
       path = "../output/Manuscript_Figs/", 
       plot = summary_PNEC_figure, width = 8, height = 5, units = "in")
...
# Data Preparation for All Tiers
prepare_data <- function(tier, erm, environment) {
  all_MC_data_long %>%
    filter(ERM == erm, Tier == tier, Environment == environment) %>%
    rename(PNEC = Value) %>%
    mutate(Environment = as.factor(Environment))
}

# Combine data for all tiers
all_tiers_data <- bind_rows(
  prepare_data("Tier2", "Food Dilution", "Marine"),
  prepare_data("Tier2", "Food Dilution", "Freshwater"),
  prepare_data("Tier2", "Tissue Translocation", "Marine"),
  prepare_data("Tier2", "Tissue Translocation", "Freshwater"),
  prepare_data("Tier3", "Food Dilution", "Marine"),
  prepare_data("Tier3", "Food Dilution", "Freshwater"),
  prepare_data("Tier3", "Tissue Translocation", "Marine"),
  prepare_data("Tier3", "Tissue Translocation", "Freshwater"),
  prepare_data("Tier4", "Food Dilution", "Marine"),
  prepare_data("Tier4", "Food Dilution", "Freshwater"),
  prepare_data("Tier4", "Tissue Translocation", "Marine"),
  prepare_data("Tier4", "Tissue Translocation", "Freshwater"),
  prepare_data("Tier1", "Food Dilution", "Marine"),
  prepare_data("Tier1", "Food Dilution", "Freshwater"),
  prepare_data("Tier1", "Tissue Translocation", "Marine"),
  prepare_data("Tier1", "Tissue Translocation", "Freshwater")
) %>%
  mutate(
    y_position = as.numeric(factor(Tier)) + 
      (as.numeric(factor(Environment)) - 0.5) * 1.5
  )

# Create the plot
# Create the plot
final_plot <- ggplot() +
  # Violin plots for Monte Carlo SSDs
  geom_violin(
    data = all_tiers_data,
    aes(x = PNEC, y = Tier, fill = Environment, color = Environment),
    draw_quantiles = c(0.05, 0.5, 0.95),
    linewidth = 1.2,
    alpha = 0.75
  ) +
  # Points for deterministic SSDs
  geom_point(
    data = base_thresholds_df,
    aes(x = PNEC, y = as.numeric(factor(Tier)) + 
          (as.numeric(factor(Environment)) - 1.2) * 0.4, fill = Environment),
    size = 5,
    shape = 22,
    color = "white",
    alpha = 0.95,
  ...
  # Error bars for deterministic SSDs
  geom_errorbarh(
    data = base_thresholds_df,
    aes(
      xmin = PNEClcl,
      xmax = PNECucl,
      y = as.numeric(factor(Tier)) + 
        (as.numeric(factor(Environment)) - 1.2) * 0.4,
      fill = Environment,
      color = Environment
    ),
    height = 0.2,
  ...
  geom_hline(
    yintercept = seq(1.5, length(unique(all_tiers_data$Tier)) - 0.5, by = 1),
    color = "gray70",
    linetype = "dashed",
    linewidth = 0.8
  ) +
  # Color scales
  scale_color_manual(values = c("Marine" = "navyblue", "Freshwater" = "seagreen")) +
  scale_fill_manual(values = c("Marine" = "navyblue", "Freshwater" = "seagreen")) +
  # Log scale for x-axis
  scale_x_log10() +
  # Labels and titles
  labs(
    title = "Comparison of Monte-Carlo and Deterministic Thresholds",
    subtitle = "Violin Plots = Monte-Carlo; Points + Error-Bars = Deterministic; All Tiers",
    fill = "Environment",
    color = "Environment",
    x = "PNEC (Particles/L; 1 to 5,000 um)",
    y = "Tier"
  ) +
  # Facet with free x-axis scales
  facet_wrap(~ERM, scales = "free_x") +
  # Themes
  ...
  theme(
    ...
    legend.position = "top",
    strip.text = element_text(size = 18, face = "bold"), # Bold and larger facet titles
    axis.title.y = element_text(size = 18, face = "bold"), # Bold and larger y-axis title
    axis.title.x = element_text(size = 18, face = "bold") # Bold and larger y-axis title
  )

# Save the plot
ggsave(
  filename = "final_plot_all_tiers.jpg",
  plot = final_plot,
  path = "../output/Manuscript_Figs/",
  width = 12,
  height = 8,
...
aoc_MC <- merge(aoc_aligned, cov_results, by = "unique_id", all.x = TRUE)
...
hist_cv_food <- aoc_MC %>% 
  ...
  drop_na(particles_L_food_dilution_deterministic) %>% 
...
hist_cv_ox <- aoc_MC %>% 
  ...
  drop_na(particles_L_ox_stress_deterministic) %>% 
...
##### CoV Distributions
```{r}
food_CoV_summary <- aoc_MC %>% 
   filter(ingestible %in% c("ingestible", "ingestible (all)", "ingestible (some)"),
         Group != "Algae") %>% 
  filter(particles_L_food_dilution_deterministic > 0) %>% 
  drop_na(particles_L_food_dilution_deterministic) %>% 
  group_by(environment) %>% 
  summarize(min = min(CoV_food_dilution, na.rm = T),
            max = max(CoV_food_dilution, na.rm = T),
            median = median(CoV_food_dilution, na.rm = T),
            sd = sd(CoV_food_dilution, na.rm = T)) %>% 
  mutate(ERM = "Food Dilution")

tissue_CoV_summary <- aoc_MC %>% 
   filter(translocatable %in% c("translocatable", "translocatable (all)", "translocatable (some)")) %>% 
  filter(particles_L_ox_stress_deterministic > 0) %>% 
  drop_na(particles_L_ox_stress_deterministic) %>% 
  group_by(environment) %>% 
  summarize(min = min(CoV_ox_stress, na.rm = T),
            max = max(CoV_ox_stress, na.rm = T),
            median = median(CoV_ox_stress, na.rm = T),
            sd = sd(CoV_ox_stress, na.rm = T)) %>% 
  mutate(ERM = "Tissue Translocation")

CoV_distribution_summary <- bind_rows(food_CoV_summary, tissue_CoV_summary) %>% 
  mutate_if(is.numeric, ~signif(., 2))

write.csv(CoV_distribution_summary, "../data/output/CoV_distribution_summary.csv")

CoV_distribution_summary
```
...
# delete existing plotly files
# unlink("../output/Manuscript_Figs/CoV_mouth_plotly_files", recursive = TRUE)
# Save the plot as an HTML file
...
### Fiter data
```{r}
# ensure ssd functions are loaded
source("utils/ssd_functions.R")
...
example.unique_id.ox.stress.max <- aoc_MC %>% 
   filter(size.length.um.used.for.conversions >= x1D_set,
         particles_L_ox_stress_deterministic > 0
         ) %>% 
...
example.unique_id.food.max <- aoc_MC %>%  
    filter(Group != "Algae",
           Species == "Hydra attenuata") %>% 
    filter(size.length.um.used.for.conversions >= x1D_set,
         particles_L_food_dilution_deterministic > 0
         ) %>% 
...
create_ggplot <- function(df_input, unique_id_input, plot_type, metric, metric_label,
  ...
  perc_01 <- quantile(df_example[[metric]], 0.001, na.rm = TRUE)
  perc_5 <- quantile(df_example[[metric]], 0.05, na.rm = TRUE)
  perc_95 <- quantile(df_example[[metric]], 0.95, na.rm = TRUE)
  perc_999 <- quantile(df_example[[metric]], 0.999, na.rm = TRUE)
  ...
  max_density <- max(density(df_example[[metric]], na.rm = TRUE)$y)
   ...
   # Calculate symmetric x-limits
  max_distance <- max(median_value - perc_01, perc_999 - median_value)
  lower_limit <- median_value - max_distance
  upper_limit <- median_value + max_distance
  ...
  p <- ggplot(df_example, aes(x = .data[[metric]])) +
    geom_histogram(aes(y = ..density..), fill = color, color = "black", alpha = 0.5, bins = 35) +
    ...
    annotate("text", x = annotation_x, y = annotation_y, 
             label = paste0(example.species, " (", example.group, ")\n",
                            "Mouth size: ", signif(example.max.size.ingest,3), " µm\n",
                            "CoV: ", example.CoV, "\n",
    ...
    theme(
     ...
     # axis.text.y = if (!y_axis_label) element_blank() else element_text(),  # Remove y-axis text for right plots
      ...
      axis.title.x = if (!x_axis_label) element_blank() else element_text(size = 10)
    )
...
plot_food_min <- create_ggplot(df_input = example.df,
                               ...
                               x_label = "Particles/L (Food Dilution; 1 to 5,000 μm)", 
                               annotation_x = 3.3E7,
                               annotation_y = 20,
                               color = "#663399",
...
plot_tissue_min <- create_ggplot(df_input = example.df,
                               ...
                               x_label = "Particles/L (Tissue Translocation; 1 to 5,000 μm)", 
                               annotation_x = 7.72e7,
                               annotation_y = 100,
                               color = "forestgreen",
...
filtered.data.small.default_t1.2_MC <- aoc_risk_paper_MC %>% 
  filter(translocatable != "not translocatable") %>% 
  filter(particles_L_ox_stress_deterministic > 0) %>% 
...
### Data preparatation
The MC Sim dataset prepared in EcoTox_MonteCarlo.Rmd contains aligned tox values for each species/group in the dataset. We can pair these datapoints to their assessement factors for time and effect metric in order to propagate uncertainty associated with that step, as is done in the PSSD.
#### Pre-process data for different ERM/Environments/Thresholds
```{r}
results_df <- readRDS("monte carlo/output/results_df_sobol.rds") %>% 
...
# global filters for all thresholds
 ...
 # filter(sim_id_numeric < 1001) #just get the first 1,000 sims
#  filter(sim_id_numeric ==1 )

########## food dilution ################
results_df_food <- results_df %>% 
  filter(ingestible != "not ingestible",
...
tier1_2_food <- readRDS("../data/output/tier1_2_food.RDS")
tier3_4_food <- readRDS("../data/output/tier3_4_food.RDS")
tier1_2_tissue <- readRDS("../data/output/tier1_2_tissue.RDS")
tier3_4_tissue <- readRDS("../data/output/tier3_4_tissue.RDS")
#application
...
##### Build functions ####
source("PSSD/rmore.r") # loads custom rmore trapezoidal distribution function by Wigger et al. (2019) (unmodified)
...
# the PSSD++ uses data-point specific CVs from Monte Caro simulations. The original Wigger et al. (2019) appraoch (i.e., "PSSD+") used a uniform CV for all data points. This constant is included here for demosntration purposes only, and is only used in a minimal example below - NOT in the actual calculations in the PSSD++
...
CV.UF <- 0.5 # Combined uncertainties of the effect metric uncertainty facotr (e.g., LOEC to NOEC, etc.) and the acute to chronic uncertainty factor from Wigger et al. (2019). This is the only arbitrary/uniform coefficient of variability included in the PSSD. 
```
...
# #example usage
tier1_2_food_matrices <- prep_data(results_df_food_marine)
```

### Run PSSD
```{r include=FALSE}
run_pSSD_analysis <- function(data_matrices, num_iterations, sim, cv_dp, cv_uf, data_name, rmore_method = "step") {
  ...
  cat(crayon::blue(sprintf("🚀 Starting PSSD analysis with %d sims each...\n", 
                           num_iterations)))
  ...
  for (i in 1:num_iterations) {
    ...
    if (i %in% checkpoints) {
      ...
      cat(crayon::yellow(sprintf("⏳ %.0f%% complete (%d of %d simulations done)\n", 
                                 pct_done, i, num_iterations)))
    }
  }
  ...
  # Save the combined results (not actually needed, as the code below includes more robust caching)
  ...
  cat(crayon::green(sprintf("✅ PSSD analysis for %s complete and saved.\n", data_name)))
...
```{r}
prepare_plot_data <- function(original_data = results_df_food_marine,
 ...
 # for (i in 1:10000) {
  for (i in 1:ncol(pSSD)) { # uncomment if not beta testing!
  ...
  if(debug){  
    ...
    tmp <- original_data %>%
      mutate(NOEC_raw = dose_new_particles_L / (af.noec * af.time),
 ...
 all_NOEC <- original_data %>% 
    ...
    group_by(Species, Group, poly_f, shape_f, effect.metric, af.noec, af.time, bio_f) %>% 
    # for each unique POD, get the 5th, 50th, and 95th percentile alignment value for plotting
    summarize(NOEC_001th = quantile(NOEC_log10, 0.0001, type = 7),
              ...
              NOEC_mean = geometric.mean(NOEC_log10, na.rm = T),
    ...
    ungroup() %>% 
    left_join(species_data %>% distinct(Species, Prop, NOEC_log10), by = "Species")

  list(quantiles_df = quantiles_df, species_data = species_data, all_NOEC = all_NOEC)
}

# Example usage
tier1_2_food_plot_prep <- prepare_plot_data(original_data = results_df_food_marine,
                                            pSSD = tier1_2_food_pssd$pSSD,
                                            data_matrices = tier1_2_food_matrices,
                                            species_data_source = aoc_final,
                                            debug = FALSE
                                            )
tier1_2_food_plot_prep
```
...
pSSD_plot_fnx <- function(quantiles_df,
                          all_NOEC,
                          species_data,
                          data_name = NULL,
                          environment_name,
  ...
  if (debug) {
    ...
    saveRDS(
      ...
      file = file.path(debug_dir, paste0("pSSD_label_x_checks_", combo_id, ".rds"))
    )
  }

  # ========== LEGEND LABELS + GROUPS ==========================================
  legend_labels <- setNames(
    paste0("<span style='color:", color_palette, "'>", names(color_palette), "</span>"),
    names(color_palette)
  )
  ...
  if (debug) cat("✅ pSSD_plot_fnx() finished for", combo_id, "\n")

  return(p)
}

# # example usage
all_species <- tier1_2_food_plot_prep$species_data %>%
  distinct(Species, Group, environment) %>%
  arrange(environment, Group, Species)

# Generate a consistent color palette
test_color_palette <- generate_color_palette(all_species)

#beepr::beep(2)

pSSD_plot_fnx(quantiles_df = tier1_2_food_plot_prep$quantiles_df,
          species_data = tier1_2_food_plot_prep$species_data,
          all_NOEC = tier1_2_food_plot_prep$all_NOEC,
          ERM_name = "Food Dilution",
...
make_PNEC_plot <- function(pssd_results, hcx, data_name, 
  ...
  PNEC <- apply(pssd_results$pSSD, 2, function(x) quantile(x, 
                                                           probs = hcx, #HC value
                                                           type = quantile_type 
                                                           )
                )
 ...
 # Calculate specific percentiles
  ...
  p999_val <- quantile(PNEC, probs = 0.999999999999, type = quantile_type, na.rm = TRUE)  # 99.9th percentile for x-axis limits
  ...
  label_data <- data.frame(
    x = c(p5_val, 
          median_val,
          (p95_val - 0.001 * x_range)),
  #  y = c(0.00001, 0.00001, 0.00001),  # Adjust y position for labels
    label = c(paste0("**5th %** <br>", signif(p5_val, 2)),
 ...
 # Plotting
  ggplot(PNEC_df, aes(x = PNEC)) +
    ...
    geom_vline(xintercept = median_val, linetype = "dotted", color = "black", size = 1.5) +
    geom_vline(xintercept = p5_val, linetype = "dotted", color = "black", size = 1) +
    geom_vline(xintercept = p95_val, linetype = "dotted", color = "black", size = 1) +
    # Add text labels for the percentiles above the distribution
  # Add text labels for the percentiles
    geom_richtext(data = label_data, aes(x = x, label = label),
              y = 0.01,
              size = 5,
                  color = "black", vjust = -1, hjust = -0.01, fill = NA, label.color = NA) +
    theme_minimal(base_size = 15) +
...
# #example
# make_PNEC_plot(pssd_results = tier1_2_food_pssd,
#            hcx = 0.05,
...
## PSSD++ (using every MC sim aligned tox value as a data point)
The following code attempts to more accurately represent the uncertainty of the alignments through the PSSD by using a distribution of aligned tox values as the data matrix, instead of using a CoV value (which necessarily requires the assumption of a distribution).


Prepare color palette for species
```{r}
all_species <- results_df %>% 
  filter(environment %in% c("Freshwater", "Marine"),
...
legend_labels <- setNames(
  paste0("<span style='color:", global_color_palette, "'>", names(global_color_palette), "</span>"),
  names(global_color_palette)
)
...
#### Preparation for each ERM/Environment/Threshold
##### Function to generate plots and summary df
```{r}
generate_plots_and_summary <- function(tier,
                                       environment,
                                       erm, 
                                       results_df,
                                       quantile_type = 7,
  ...
  # ---------- STEP 1: Prepare data ----------
  MCdf <- tryCatch({
    if (debug) cat("🔧 STEP 1: prep_data()...\n")
    x <- prep_data(results_df)
    if (debug) {
      cat("  MCdf names:", paste(names(x), collapse = ", "), "\n")
      saveRDS(x, file = file.path(debug_dir, paste0("step1_MCdf_", combo_id, ".rds")))
    }
    x
  }, error = function(e) {
    if (debug) {
      saveRDS(list(
        error = e,
        results_df = results_df
      ), file = file.path(debug_dir, paste0("ERROR_step1_prep_data_", combo_id, ".rds")))
  ...
  pssd_results <- tryCatch({
    if (debug) cat("🔧 STEP 2: run_pSSD_analysis()...\n")
    x <- run_pSSD_analysis(
      data_matrices = MCdf,
      num_iterations = num_iterations,
      sim = sim,
    ...
    if (debug) {
      ...
      saveRDS(x, file = file.path(debug_dir, paste0("step2_pssd_results_", combo_id, ".rds")))
    }
    x
  }, error = function(e) {
    if (debug) {
      saveRDS(list(
        error = e,
        MCdf = MCdf
      ), file = file.path(debug_dir, paste0("ERROR_step2_pssd_", combo_id, ".rds")))
  ...
  plot_prep <- tryCatch({
    if (debug) cat("🔧 STEP 3: prepare_plot_data()...\n")
    x <- prepare_plot_data(
    ...
    if (debug) {
      ...
      saveRDS(x, file = file.path(debug_dir, paste0("step3_plot_prep_", combo_id, ".rds")))
    }
    x
  }, error = function(e) {
    if (debug) {
      saveRDS(list(
        ...
        MCdf = MCdf,
        results_df = results_df
      ), file = file.path(debug_dir, paste0("ERROR_step3_prepare_plot_data_", combo_id, ".rds")))
  ...
  pSSD_plot <- tryCatch({
    if (debug) cat("🔧 STEP 4a: pSSD_plot_fnx()...\n")
    x <- pSSD_plot_fnx(
    ...
    if (debug) {
      saveRDS(x, file = file.path(debug_dir, paste0("step4a_pSSD_plot_", combo_id, ".rds")))
    }
    x
  }, error = function(e) {
    if (debug) {
      saveRDS(list(
        ...
        species_data = plot_prep$species_data,
        all_NOEC = plot_prep$all_NOEC
      ), file = file.path(debug_dir, paste0("ERROR_step4a_pSSD_plot_", combo_id, ".rds")))
  ...
  if (debug) {
    saveRDS(PNEC_05_df, file = file.path(debug_dir, paste0("step4b_PNEC05_df_", combo_id, ".rds")))
  }

  PNEC_plot_05 <- tryCatch({
    if (debug) cat("🔧 STEP 4b: make_PNEC_plot() HC5...\n")
    p <- make_PNEC_plot(
      ...
      labs(title  = "Tier 3 (HC5)",
           x = "PNEC (Particles/L; 1 to 5,000 μm)") +
      theme(axis.text.y = element_blank())
    if (debug) saveRDS(p, file = file.path(debug_dir, paste0("step4b_PNEC_plot_05_", combo_id, ".rds")))
    p
  }, error = function(e) {
    if (debug) {
      saveRDS(list(error = e, pssd_results = pssd_results),
              file = file.path(debug_dir, paste0("ERROR_step4b_PNEC_plot_05_", combo_id, ".rds")))
  ...
  PNEC_plot_10 <- tryCatch({
    if (debug) cat("🔧 STEP 4b: make_PNEC_plot() HC10...\n")
    p <- make_PNEC_plot(
      pssd_results = pssd_results,
      hcx = 0.10,
      quantile_type = quantile_type,
      ...
      labs(title = "Tier 4 (HC10)",
           x = "PNEC (Particles/L; 1 to 5,000 μm)") +
      theme(axis.title.y = element_blank(),
            axis.text.y = element_blank())
    if (debug) saveRDS(p, file = file.path(debug_dir, paste0("step4b_PNEC_plot_10_", combo_id, ".rds")))
    p
  }, error = function(e) {
    if (debug) {
      saveRDS(list(error = e, pssd_results = pssd_results),
              file = file.path(debug_dir, paste0("ERROR_step4b_PNEC_plot_10_", combo_id, ".rds")))
  ...
  if (debug) {
    saveRDS(list(PNEC_summary_05 = PNEC_summary_05,
                 PNEC_summary_10 = PNEC_summary_10),
            file = file.path(debug_dir, paste0("step5_PNEC_summaries_", combo_id, ".rds")))
  }

  # ---------- STEP 6: Arrange plots ----------
  arranged_plot <- tryCatch({
    if (debug) cat("🔧 STEP 6: ggarrange()...\n")
    p <- ggarrange(
      pSSD_plot, 
    ...
    if (debug) saveRDS(p, file = file.path(debug_dir, paste0("step6_arranged_plot_", combo_id, ".rds")))
    p
  }, error = function(e) {
    if (debug) {
      saveRDS(list(
      ...
      ), file = file.path(debug_dir, paste0("ERROR_step6_arranged_plot_", combo_id, ".rds")))
  ...
  # ---------- STEP 7: Save figures ----------
  tryCatch({
    if (debug) cat("💾 STEP 7: ggsave() manuscript...\n")
    ggsave(
    ...
    if (debug) {
      saveRDS(list(error = e, arranged_plot = arranged_plot),
              file = file.path(debug_dir, paste0("ERROR_step7_ggsave_manuscript_", combo_id, ".rds")))
      cat("❌ ERROR @ STEP 7 (ggsave manuscript):", e$message, "\n")
    }
    stop(e)
  })

  tryCatch({
    if (debug) cat("💾 STEP 7: ggsave() presentation...\n")
    ggsave(
      filename = paste0(erm, "_", environment, "_arranged_plot.jpg"),
      dpi = 450,
    ...
    if (debug) {
      saveRDS(list(error = e, arranged_plot = arranged_plot),
              file = file.path(debug_dir, paste0("ERROR_step7_ggsave_presentation_", combo_id, ".rds")))
      cat("❌ ERROR @ STEP 7 (ggsave presentation):", e$message, "\n")
    }
    stop(e)
  })

  # ---------- RETURN ----------
  if (debug) cat("✅ generate_plots_and_summary() complete for", combo_id, "\n")
...
get_results_df <- function(tier, environment, erm) {
  if (environment == "Marine" && erm == "Food Dilution") {
    if (tier < 3) return(results_df_food_marine)
    else return(results_df_food_t3_t4_marine)
  } else if (environment == "Marine" && erm == "Tissue Translocation") {
    if (tier < 3) return(results_df_tissue_marine)
    else return(results_df_tissue_t3_t4_marine)
  } else if (environment == "Freshwater" && erm == "Food Dilution") {
    if (tier < 3) return(results_df_food_freshwater)
    else return(results_df_food_t3_t4_freshwater)
  } else if (environment == "Freshwater" && erm == "Tissue Translocation") {
    if (tier < 3) return(results_df_tissue_freshwater)
    else return(results_df_tissue_t3_t4_freshwater)
  } else {
...
##### For loop to apply function to each combo of ERM/Environment/Tier
```{r}
# save copy
...
```{r}
#### BETA TESTING ### 
### COMMENT WHEN RUNNING FULL SIM!!! ####
# results_df contains 10,000 simulations. This will cause an extremely long runtime for the PSSD. We need to subsample to beta test. sample by simulation_id to n = 10

# print max simulation_id in results_df_OG
cat("Max simulation_id in results_df_OG:", max(results_df_OG$simulation_id), "\n")

# subset 100 random sims for beta-testing
results_df_test <- results_df_OG %>%
  group_by(simulation_id) %>%
  #sample_n(3) %>%
  ungroup()

# print size of results_df before and after sampling
cat("Size of results_df before sampling:", comma(nrow(results_df_OG)), "\n")
cat("Size of results_df after sampling:", comma(nrow(results_df_test)), "\n")


# overwrite results_df with test set to avoid having to alter downstream code
results_df <- results_df_test
# this set is 173 times smaller
```

*For Loop PSSD+ with Caching*

* Each simulation result is saved to disk immediately after it's run, rather than only saving the full object at the end.
* When the function starts, it checks if cached results already exist.
* It loads any cached results and only runs simulations for the missing iterations.
* After the loop is complete, it combines the cached pieces and saves the final result as usual.
...
output_path <- "../output/Manuscript_Figs/"
presentation_path <- "../output/presentation_Figs/"
sim <- 300 # Number of simulations to run (300 default for PSSD++ based on 64 GB RAM capacity)
num_iterations <- sim
cv_uf <- 0.5
#rmore_method <- "step" #'step' (original Wigger Trapezoidal) or 'lognormal' (shortcut developed here=)
rmore_method <- "lognormal" #'step' (original Wigger Trapezoidal) or 'lognormal' (shortcut developed here=)
quantile_type <- 8 #linear interpolation for the median-unbiased estimate to determine the values (i - 3/8) / (n + ¼; where i is the rank, and n is the number of observations). This ensures that distinct values are produced for every HCx (e.g., HC5, HC10), even with limited data.
species_data_source <- aoc_aligned
debug_option = FALSE #debugging option provides intermediate outputs and diagnostics to assist the user if the function provides unexcpected or incomplete results. This option can significantly increase processing time, so it's advised to run this with the minimum number of simulations (i.e., sim = 3)
base_cache_dir <- "../output/pssd_cache/"
  ...
  for (environment in environments) {
    for (erm in erms) {
      ...
      if (file.exists(cache_file)) {
        cat(blue(sprintf("\n🔁 [%s] Skipping (cached): %s\n", format(iteration_start_time, "%H:%M:%S"), combo_id)))
        results[[combo_id]] <- readRDS(cache_file)
        next
      }

      cat(green(sprintf("\n⏳ [%s] Starting: Tier %d | Environment: %s | ERM: %s\n", 
                        format(iteration_start_time, "%H:%M:%S"), tier, environment, erm)))
      ...
      tryCatch({
        ...
        iteration_time_sec <- as.numeric(difftime(iteration_end_time, iteration_start_time, units = "secs"))
        iteration_times[iteration_count] <- iteration_time_sec
        
        running_average_sec <- mean(iteration_times[1:iteration_count])
        est_time_remaining_sec <- running_average_sec * (total_iterations - iteration_count)

        cat(yellow(sprintf(
          "✅ Completed: %s in %.2f hr | Est. remaining: %.2f hr\n", 
          combo_id, iteration_time_sec / 3600, est_time_remaining_sec / 3600
        )))
        
      }, error = function(e) {
        beepr::beep(sound = 7)  # ⚠️ Distinct error sound
...
```{r}
results$`Tier3_Marine_Food Dilution`$quantiles
```


#### Summary table
```{r}
saveRDS(results, "../data/output/PSDDplusplusresults.rds")
```
...
PNEC_summary_table <- PNEC_summary_table[, c("Tier", "Environment", "ERM", "HCX", "Statistic", "Value")]

# Sort the table by Environment, ERM, Tier, and HCX
PNEC_summary_table <- PNEC_summary_table[order(PNEC_summary_table$Environment, PNEC_summary_table$ERM, PNEC_summary_table$Tier, PNEC_summary_table$HCX), ]
...
saveRDS(PNEC_summary_table_wide,
        "../data/output/PNEC_summary_table_wide.rds"
        )
...
plot_list <- list(
  # top left
    results$`Tier3_Marine_Food Dilution`$pSSD_plot +
      labs(title = "Marine", y = "Food Dilution") + # Explicitly set y-axis label
    scale_color_manual(values = global_color_palette, 
                       labels = legend_labels,
                       drop = FALSE,
                       name = "Organism Group") + # Apply consistent color mapping
    scale_fill_manual(values = global_color_palette, 
                      labels = legend_labels,
                      drop = FALSE,
                       name = "Organism Group") + # Apply consistent color mapping
    theme(axis.title.y = element_text(size = 18, face = "bold"), # Keep y-axis title
          plot.title = element_text(size = 20, face = "bold"),
          plot.subtitle = element_blank(),
          axis.title.x = element_blank(),
          axis.text.y = element_text(size = 10),
          legend.text = element_markdown(size = 12)), # Keep y-axis text
  # top right
  results$`Tier3_Freshwater_Food Dilution`$pSSD_plot +
    ...
    theme(axis.title.y.right = element_text(size = 13), #cumulative probability title on right
          plot.title = element_text(size = 20, face = "bold"),
          plot.subtitle = element_blank(),
          axis.title.x = element_blank(),
          axis.text.y = element_text(size = 10),
          legend.text = element_markdown(size = 12)), # Keep y-axis text
  # bottom left
  results$`Tier3_Marine_Tissue Translocation`$pSSD_plot +
    labs(y = "Tissue Translocation") +
    scale_color_manual(values = global_color_palette, 
                       labels = legend_labels,
                       drop = FALSE,
                       name = "Organism Group") + # Apply consistent color mapping
    ...
    theme(axis.title.y = element_text(size = 18, face = "bold"), # Keep y-axis title
          ...
          legend.text = element_markdown(size = 12)), # Keep y-axis text
  # bottom right
  results$`Tier3_Freshwater_Tissue Translocation`$pSSD_plot +
    scale_y_continuous(position = "right") +
    scale_color_manual(values = global_color_palette, 
                       labels = legend_labels,
                       drop = FALSE,
    ...
    scale_fill_manual(values = global_color_palette, 
                      ...
                      drop = FALSE,
                       name = "Organism Group") + # Apply consistent color mapping
    theme(axis.title.y.right = element_text(size = 13), # Remove y-axis title
          plot.subtitle = element_blank(),
          axis.title.x = element_text(size = 12),
          plot.title = element_blank(),
          axis.text.y = element_text(size = 10),
...
arranged_plot <- ggarrange(
  plotlist = plot_list,
  ncol = 2, nrow = 2, # Arrange in 2x2 grid
  common.legend = TRUE, # Common legend for all plots
  legend = "bottom" # Place legend at the bottom
) +
  guides(color = guide_legend(override.aes = list(shape = 21, size = 3,  hjust = NULL, color = NA, alpha = 1,
                                                  fill = unname(global_color_palette))),
         fill =  guide_legend(override.aes = list(shape = 21, size = 3,  hjust = NULL, color = NA, alpha = 1,
                                                  fill = unname(global_color_palette)))
         )   # Combine color and fill legends

# Save the final plot
ggsave(
  filename = "../output/Manuscript_Figs/Figure5.jpg",
  plot = arranged_plot,
  dpi = 500,
  width = 9,
  height = 7,
...
PNEC_plot_list <- list(
  ...
  results$`Tier3_Marine_Food Dilution`$PNEC_plot_05 +
   labs(title = "Marine", y = "Food Dilution") + # Explicitly set y-axis label
    theme(axis.title.y = element_text(size = 16, face = "bold"), # Keep y-axis title
          plot.subtitle = element_blank(),
          axis.title.x = element_text(size = 12),
          axis.text.y = element_text(size = 10)), # Keep y-axis text
  
  results$`Tier3_Freshwater_Food Dilution`$PNEC_plot_05 +
    labs(title = "Freshwater",
         subtitle = "") +
   ...
   results$`Tier3_Marine_Tissue Translocation`$PNEC_plot_05 +
    labs(title = "Tissue Translocatiion") +
    theme(axis.title.y = element_text(size = 16, face = "bold"), # Keep y-axis title
          plot.subtitle = element_blank(),
          axis.title.x = element_blank(),
          axis.text.y = element_text(size = 10)), # Keep y-axis text
  
  results$`Tier3_Freshwater_Tissue Translocation`$PNEC_plot_05 +
    theme(axis.title.y = element_text(size = 12), # Remove y-axis title
          plot.title = element_blank(),
          plot.subtitle = element_blank(),
          axis.title.x = element_blank(),
          axis.text.y = element_text(size = 10)) # Keep y-axis text
...
# Arrange the plots
PNEC_arranged_plot <- ggarrange(
  plotlist = PNEC_plot_list,
  #labels = c("A", "B", "C", "D"),
  ncol = 2, nrow = 2, # Arrange in 2x2 grid
  common.legend = TRUE, # Common legend for all plots
  legend = "bottom" # Place legend at the bottom
...
# Save the final plot
ggsave(
  filename = "../output/Manuscript_Figs/PNEC_final_plot.jpg",
  plot = PNEC_arranged_plot,
  dpi = 500,
  width = 10,
  height = 8,
  units = "in"
...
# Display the final plot
print(PNEC_arranged_plot)
```

##### Comparison of pSSD and SSD-MC Distributions (Figure 6)
```{r}
SSD_base_thresholds <- base_thresholds_df %>% 
  mutate(y_position_comparison = case_when(
    Environment == "Marine" & ERM == "Tissue Translocation" ~ 1.9,
    Environment == "Marine" & ERM == "Food Dilution" ~ 0.8,
    Environment == "Freshwater" & ERM == "Tissue Translocation" ~ 4.1,
    Environment == "Freshwater" & ERM == "Food Dilution" ~ 0.8
  ))

# MC data
marine_t3_tissue_MC <- all_tiers_data %>% 
                   filter(Tier == "Tier3", Environment == "Marine", ERM == "Tissue Translocation")

# pSSD data
marine_t3_tissue_pSSD <- results$`Tier3_Marine_Tissue Translocation`$PNEC_df

# custom labels fuinction
labels_function = function(x) {
  superscript <- c(
    "0" = "\u2070", "1" = "\u00B9", "2" = "\u00B2", "3" = "\u00B3",
    "4" = "\u2074", "5" = "\u2075", "6" = "\u2076", "7" = "\u2077",
    "8" = "\u2078", "9" = "\u2079", "-" = "\u207B"
  ...
  sapply(x, function(v) {
    ...
    if (v < 0.01 || v > 100) {
      ...
      if (length(parts) != 2) return(as.character(v))
      
      coeff <- parts[1]
      exponent <- gsub("^\\+?(-?)0*", "\\1", parts[2])  # Remove leading 0s and optional +
      exp_chars <- strsplit(exponent, "")[[1]]
      
      # Build superscript string
      exp_sup <- paste0(superscript[exp_chars], collapse = "")
      paste0(coeff, "\u00D7", "10", exp_sup)  # × = \u00D7
    } else {
...
# Define dataset mapping for pSSD
get_PNECs_df <- function(tier, environment, erm) {
  if (environment == "Marine" && erm == "Tissue Translocation") {
    if (tier == "Tier3") return(results$`Tier3_Marine_Tissue Translocation`$PNEC_df)
    if (tier == "Tier4") return(results$`Tier4_Marine_Tissue Translocation`$PNEC_df)
  } else if (environment == "Marine" && erm == "Food Dilution") {
    if (tier == "Tier3") return(results$`Tier3_Marine_Food Dilution`$PNEC_df)
    if (tier == "Tier4") return(results$`Tier4_Marine_Food Dilution`$PNEC_df)
  } else if (environment == "Freshwater" && erm == "Tissue Translocation") {
    if (tier == "Tier3") return(results$`Tier3_Freshwater_Tissue Translocation`$PNEC_df)
    if (tier == "Tier4") return(results$`Tier4_Freshwater_Tissue Translocation`$PNEC_df)
  } else if (environment == "Freshwater" && erm == "Food Dilution") {
    if (tier == "Tier3") return(results$`Tier3_Freshwater_Food Dilution`$PNEC_df)
    if (tier == "Tier4") return(results$`Tier4_Freshwater_Food Dilution`$PNEC_df)
  } else {
    stop("Invalid combination of tier, environment, and ERM")
  }
}
...
#### FUNCTION TO MAKE COMPARISON PLOTS of PSSD and MC PNECs ####
comparison_plot_function <- function(erm = "Tissue Translocation",
                                     environment = "Marine",
                                     tier = "Tier3",
                                     pSSD_df = results,
                                     MC_df = all_tiers_data){
#prep MC data  
MC_PNECs <- MC_df %>% 
  filter(Tier == tier,
         Environment == environment,
...
# prep pSSD data
pSSD_PNECs <- get_PNECs_df(tier = tier, environment = environment, erm = erm)

#prep SSD data
SSD_PNECs <- SSD_base_thresholds %>% filter(Tier == 3, Environment == environment, ERM == erm)

#define xlimits
xlimits <- c(quantile(pSSD_PNECs$PNEC, 0.00001),
             quantile(pSSD_PNECs$PNEC, 0.99999))
  
ggplot() +
  ###### SSD PNECs ####
  geom_point(data = SSD_PNECs, 
             aes(x = PNEC, y = y_position_comparison,
                 fill = factor("SSD", levels = c("SSD", "MC-SSD", "PSSD++")), 
                 color = factor("SSD", levels = c("SSD", "MC-SSD", "PSSD++"))),
                 size = 5, shape = 22, 
                 #color = "white",
                 alpha = 0.95,
                 stroke = 1.8 #thickness of outline
                 ) +
   geom_errorbarh(data = SSD_PNECs, 
                  aes(xmin = PNEClcl, xmax = PNECucl, y = y_position_comparison,
                      fill = factor("SSD", levels = c("SSD", "MC-SSD", "PSSD++")),
                      color = factor("SSD", levels = c("SSD", "MC-SSD", "PSSD++"))
                      ),
                  height = 0.2,
                  ...
                  linewidth = 1.2) +
  ###### MC-SSD PNECs #####
  geom_density(data = MC_PNECs, 
             aes(x = PNEC, 
                 color = factor("MC-SSD", levels = c("SSD", "MC-SSD", "PSSD++")), 
                 fill = factor("MC-SSD", levels = c("SSD", "MC-SSD", "PSSD++"))), 
             lwd = 1, alpha = 0.5) +

  ###### pSSD PNECs #####
  geom_density(data = pSSD_PNECs, 
             aes(x = PNEC, 
                 color = factor("PSSD++", levels = c("SSD", "MC-SSD", "PSSD++")), 
                 fill = factor("PSSD++", levels = c("SSD", "MC-SSD", "PSSD++"))), 
             lwd = 1, alpha = 0.5) +

  # Adjust x-axis limits and scale
scale_x_log10(
  limits = xlimits,
  breaks = c(0.001, 0.01, 0.1, 1, 10, 100, 1000, 1e4, 1e5),
  labels = labels_function) +


  # Define custom colors for the legend
  scale_fill_manual(name = "Method", 
                    values = c("MC-SSD" = "#1F77B4", "SSD" = "#117733", "PSSD++" = "#EE3377")) +
  ...
  labs(title = environment, y = "Relative Density", x = "PNEC \n(Tier 3; Particles/L; 1 to 5,000 μm)")

}

### make ggplots for each combo

PNECs_comparisons <- list()

tiers <- c("Tier3")
erms <- c("Food Dilution", "Tissue Translocation")
environments = c("Marine", "Freshwater")
...
for (tier in tiers) {
  for (environment in environments) {
    for (erm in erms) {
      ...
      # make plot    
      PNEC_plot <- comparison_plot_function(erm = erm, environment = environment, tier = tier)
      
      #store results
    PNECs_comparisons[[paste0("Tier", tier, "_", environment, "_", erm)]] <- PNEC_plot
    }
  }
}
...
#### arrange plots
# Customize the plots
PNEC_compare_plot_list <- list(
  PNECs_comparisons$`TierTier3_Marine_Food Dilution` +
    labs(title = "Marine") +
    ylab("Food Dilution") + # Add y-axis title
    theme(axis.title.y = element_text(size = 16, face = "bold"), # Keep y-axis title
          plot.subtitle = element_blank(),
          axis.title.x = element_blank()
          #axis.text.y = element_text(size = 10)
  ...
  PNECs_comparisons$`TierTier3_Freshwater_Food Dilution` +
    labs(title = "Freshwater") +
    scale_x_log10(
    limits = c(0.01, 1e4),
    breaks = c(0.01, 1, 100, 1e4),
    labels = labels_function  # The function we just wrote
  ) +
    scale_y_continuous(position = "right") +
    theme(axis.title.y = element_text(size = 12),
          plot.subtitle = element_blank(),
   ...
   PNECs_comparisons$`TierTier3_Marine_Tissue Translocation` +
    scale_x_log10(
    limits = c(10, 1e4),
    breaks = c(10, 100, 1000, 1e4),
    labels = labels_function  # The function we just wrote
  ) +
 labs(y = "Tissue Translocation") + # Explicitly set y-axis label
    theme(axis.title.y = element_text(size = 16, face = "bold"), # Keep y-axis title
          plot.subtitle = element_blank(),
          plot.title = element_blank(),
          axis.title.x = element_text(size = 12)
  ...
  PNECs_comparisons$`TierTier3_Freshwater_Tissue Translocation` +
    labs(subtitle = "") +
    scale_y_continuous(position = "right") +
    scale_x_log10(
    limits = c(100, 1e5),
...
# Arrange the plots
PNEC_compare_arranged_plot <- ggarrange(
  plotlist = PNEC_compare_plot_list,
  ncol = 2, nrow = 2, # Arrange in 2x2 grid
  common.legend = TRUE, # Common legend for all plots
  legend = "bottom" # Place legend at the bottom
)
...
# Save the final plot
ggsave(
  #filename = "../output/Manuscript_Figs/PNEC_compare_arranged_plot_trapezoidal.jpg", # uncomment if doing comparison with lognormal/trapezoidal. Figure 6 uses trapezoidal method.
  filename = "../output/Manuscript_Figs/Figure6_PNEC_compare_arranged_plot.jpg",
  plot = PNEC_compare_arranged_plot,
  dpi = 500,
  width = 7.5,
  height = 5.5,
  units = "in"
...
# Display the final plot
print(PNEC_compare_arranged_plot)
```
...
tomex2.0_aoc_z_final_utf8 <- stringi::stri_enc_toutf8(aoc_aligned_no_filters, validate = TRUE)

# List of data frame names
dataframe_names <- c("full_summary", "formatted_thresholds", "simple_thresholds_MC", "simple_thresholds_MC_mass", "MC_RSD",
                     "PNEC_summary_table_wide",
                     "freshwater_marine_params_species_sensitivity_analysis",
                     "unified_thresholds", "leave_one_out_df", "tomex2.0_aoc_z_final_utf8")

# Corresponding custom sheet names
sheet_names <- c("Database summary",
                 "Deterministic_thresholds", "Monte_Carlo_thresholds", "Monte_Carlo_thresholds_mass", "Monte_Carlo_RSD", 
                 "PSSD++PNEC_summary",
                 "Fresh_Mar_Sp_Par_sensitivity", "Thrshlds_Tr_ERM_Env_Qulty", "Leave_one_out_studies", "Full ToMEx2.0 dataset")
