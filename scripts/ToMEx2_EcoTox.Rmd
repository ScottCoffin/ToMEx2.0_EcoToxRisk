...
tier3_4_tissue <- filtered.data.large.default_t3.4
```
...
prep_data <- function(data) {
  ...
  matrices <- list(
    ...
    UFdd = data %>% mutate(id = row_number()) %>% 
        ...
        Matrix(sparse = T),
    doseDescriptor <- t2 %>% mutate(id = row_number()) %>% 
      select(id, Species, effect.metric) %>% 
      acast(id ~ Species, value.var = "effect.metric") %>% 
      Matrix(sparse = T),
    polymer_matrix <- t2 %>% mutate(id = row_number()) %>% 
    ...
    shape__matrix <- t2 %>% 
      ...
      Matrix(sparse = T),
    env_1_large_volume <- t2 %>% mutate(id = row_number()) %>% 
      select(id, Species, environment) %>% 
      acast(id ~ Species, value.var = "environment") %>% 
      Matrix(sparse = T),
    group_1_large_volume <- t2 %>% mutate(id = row_number()) %>% 
...
PNEC_plot <- function(pssd_results, data_name){
  PNEC <- apply(pssd_results$pSSD, 2, function(x) quantile(x, probs = 0.05, type = 1))
...
#Step 4b: generate ggplot (PNEC)
tier3_4_food_PNECPlot <-  PNEC_plot(pssd_results = tier3_4_food_pssd,
                                    data_name = "Tier 3 (Food Dilution)")
...
```{r}
ggarrange(tier3_4_food_pssdPlot, tier3_4_food_PNECPlot,
          ncol = 1)
```
```{r}
...
#Step 4b: generate ggplot (PNEC)
tier3_4_tissue_PNECPlot <-  PNEC_plot(pssd_results = tier3_4_tissue_pssd,
                                    data_name = "Tier 3 (Tissue Translocation)")
...
```{r}
ggarrange(tier3_4_tissue_pssdPlot,tier3_4_tissue_PNECPlot,
          ncol = 1)
```
