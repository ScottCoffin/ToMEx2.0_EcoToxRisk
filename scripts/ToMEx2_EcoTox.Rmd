...
tier1_2_food_PNECPlot <-  make_PNEC_plot(pssd_results = tier1_2_food_pssd,
                                    hcx = 0.05,
                                    data_name = "Tier 2 (Food Dilution)")
...
#Step 4b: generate ggplot (PNEC)
tier3_food_PNECPlot <-  make_PNEC_plot(pssd_results = tier3_4_food_pssd,
                                    hcx = 0.05,
                                    data_name = "Tier 3 (Food Dilution)")

tier4_food_PNECPlot <-  make_PNEC_plot(pssd_results = tier3_4_food_pssd,
                                    hcx = 0.10,
...
#Step 4b: generate ggplot (PNEC)
tier3_tissue_PNECPlot <-  make_PNEC_plot(pssd_results = tier3_4_tissue_pssd,
                                      hcx = 0.05,
                                      data_name = "Tier 3 (Tissue Translocation)")

tier4_tissue_PNECPlot <-  make_PNEC_plot(pssd_results = tier3_4_tissue_pssd,
                                      hcx = 0.10,
                                      data_name = "Tier 3 (Tissue Translocation)")
...
PNEC_pSSDvSSD <- ggplot() +
   ...
   geom_jitter(data = PNEC_dfs, aes(x = PNEC, y = y_position, fill = ERM, color = ERM),
               height = 0.15, size = 1.5, alpha = 0.03) +  # Jittered points
   geom_point(data = base_thresholds_df_marine %>% filter(Tier>2), 
              aes(x = PNEC, y = y_position, fill = ERM),
              size = 5, shape = 23, 
              color = "white",
              alpha = 0.95,
   ...
   geom_errorbarh(data = base_thresholds_df_marine %>% filter(Tier>2), 
                  aes(xmin = PNEClcl, xmax = PNECucl, y = y_position, fill = ERM, color = ERM),
                  height = 0.2,
                  alpha = 0.75,
                  linewidth = 1.2) +
  ...
  xlab("PNEC (Particles/L; 1 to 5,000 μm)") +
  ylab("Threshold Tier")
...
sim <- 3 #number of simulations in pSSD. 3 is minimum. I've tried 1,000, but it creates a matrix too big for my RAM (112 GB). It appears that we need to keep this value less than 500 or so for most machines to be able to run (300 seems to be a sweet spot).
num_iterations <- sim
