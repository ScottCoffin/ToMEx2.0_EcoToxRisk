...
aoc_final <- aoc_intermediate_alphas  %>% 
   # define environment-specific alpha parameters #
    mutate(alpha = case_when(environment == "Marine" ~ alpha.marine,
                             environment == "Freshwater" ~ alpha.marine),
           a.sa = case_when(environment == "Marine" ~ a.sa.marine,
                            environment == "Freshwater" ~ a.sa.marine),
           a.v = case_when(environment == "Marine" ~ a.v.marine,
                            environment == "Freshwater" ~ a.v.marine),
           a.m = case_when(environment == "Marine" ~ a.m.marine,
                           environment == "Freshwater" ~ a.m.marine),
           a.ssa = case_when(environment == "Marine" ~ a.ssa.marine,
                           environment == "Freshwater" ~ a.ssa.marine),
           R.ave = case_when(environment == "Marine" ~ R.ave.water.marine,
                             environment == "Freshwater" ~ R.ave.water.marine),
           p.ave = case_when(environment == "Marine" ~ p.ave.marine,
                             environment == "Freshwater" ~ p.ave.marine)
           ) %>% 
  ###### re-calculate size, surface area, volume, mass based on user-defined R.ave ####
#### Note that in this instance, we are using marine values across the board - regardles of species! #
...
#fit distributions
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c( "llogis", "lnorm", 
                                                            "weibull",
                                                            #"gamma",
                                                            "lgumbel"#, "burrIII3"
                                                            ), 
...
dists <- ssd_fit_dists(collapsed, left = "Conc", 
                       dists = c( "llogis", "lnorm", #"burrIII3", #"gamma",
                                  "weibull",
...
#fit distributions
 if(n_distinct(collapsed$Species) > 5){
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c( "llogis", "lnorm", #"burrIII3",#"gamma",
                                                            "weibull",
                                                            "lgumbel"), computable = FALSE, silent = FALSE) 
#use average distribution with weighthing based on AICC
preds <- predict(dists, average = TRUE, 
...
aoc_risk_paper <- aoc_final %>% 
  ...
  mutate(AF.total = af.time * af.noec) %>% 
...
##### Summarized
```{r}
filtered.data.small.default_t1.2 %>% 
   group_by(Species, Group) %>%
...
```{r}
filtered.data.small.default_t1.2 %>% 
  mutate(dose_new_particles_mL = signif(dose_new / 1000), 3) %>% 
  dplyr::select(Group, Species, #environment,
                ...
                EC_mono_p.particles.mL_ingest, mu.v.mono,
  ...
  arrange(dose_new_particles_mL)# %>%  
   #filter(Species == "Parvocalanus crassirostris")
```
...
#no filters
aoc_aligned_no_filters <-  tomex2.0_aoc_z_final %>% 
  filter(!environment %in% c("Terrestrial", "Not Reported"),
         Group != "Bacterium",
         Group != "Plant",
         effect.metric != "HONEC") 
