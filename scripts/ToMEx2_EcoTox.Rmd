...
#mouth size openings
#max.size.ingest.mm=10^(0.9341⋅log10(body.length.cm)−1.1200)
# Coefficients
...
## tissue translocation parameters derived in Mehinto et al (2022) See SI:
# Model parameters for the binomial regression were as follows: intercept = 1.24 (+- 0.40 [standard error]; p = 0.002); exp(particle length [µm]) = -0.014 +- 0.007  [standard error]; p  = 0.034).
```

#### Test to prove it provides same results
...
for (i in 1:n_sim) {
  ...
  #parameters to iterate
   sim_beta_log10_body_length <- sim_beta_log10_body_length[1]
   sim_body_length_intercept <- sim_body_length_intercept[1]
   
# calculate ERM for each species
aoc_final <- aoc_intermediate  %>% 
  ungroup() %>% 
  select(c(rowid, body.length.cm, max.size.ingest.um, max.size.ingest.mm)) %>% 
  ...
  #Monte Carlo
  mutate(max.size.ingest.mm.sim = 10^(sim_beta_log10_body_length * log10(body.length.cm) - sim_body_length_intercept) * 10)
%>% #jamm et al (2020) Nature
  mutate(max.size.ingest.um.sim = 1000* max.size.ingest.mm.sim)
  
   # Store results in a list
  MC_results[[i]] <- list(max.size.ingest.um.sim = aoc_final$max.size.ingest.um.sim,
                          rowid = aoc_final$rowid#,
                         # max.size.ingest.um.det = aoc_final$max.size.ingest.um
                          #aoc_final$max.size.ingest.um
                          )
}
...
results_df <- do.call(rbind, lapply(MC_results, function(x) {
  data.frame(
    rowid = x$rowid,
    max.size.ingest.um.sim = x$max.size.ingest.um.sim#,
   # max.size.ingest.um.det = x$max.size.ingest.um
  )
}))

max.size.ingest.um_MC_results <- results_df %>%
  group_by(rowid) %>%
  summarise(sd_max.size.ingest.um.sim = sd(max.size.ingest.um.sim, na.rm = TRUE),
            mean_max.size.ingest.um.sim = mean(max.size.ingest.um.sim, na.rm = TRUE),
            CoV_max.size.ingest.um.sim = sd_max.size.ingest.um.sim / mean_max.size.ingest.um.sim
  )

# View the results
print(max.size.ingest.um_MC_results)

max.size.ingest.um_MC_results
```
...
source("functions.R") # necessary for surface area, volume calculations
  ...
  #  a.sa <- median(a.sa_samples)
  #  a.v <- median(a.v_samples)
  #  a.m <- median(a.m_samples)
  #  a.ssa <- median(a.ssa_samples)
  #  R.ave <- median(R.ave_samples) #length to width ratio (environment-specific)
  # # p.ave <- p.ave_samples[i] #not used
  #  sim_beta_log10_body_length <- median(sim_beta_log10_body_length_samples)
  #  sim_body_length_intercept <- median(sim_body_length_intercept_samples)
...
R.ave = 0.77 #average width to length ratio for microplastics in marine enviornment
p.ave = 1.10 #average density in marine surface water
sim_beta_log10_body_length = 0.9341
sim_body_length_intercept = 1.1200
  
# calculate ERM for each species
aoc_deterministic <- aoc_intermediate  %>% 
  ungroup() %>%
     # # particle parameters are calculated in the tidying script in the RShiny app. Since we're doing this probabilistically here, we'll need to remove those parameters and recalculate.
     # select(-c(size.width.min.um.used.for.conversions, size.width.max.um.used.for.conversions,
     #           size.height.min.um.used.for.conversions, size.height.max.um.used.for.conversions,
     #           size.length.max.um.used.for.conversions,size.length.min.um.used.for.conversions,
     #           particle.surface.area.um2.max, particle.surface.area.um2.min, particle.surface.area.um2,
     #           particle.volume.um3.min, particle.volume.um3.max, particle.volume.um3,  
     #           mass.per.particle.mg.min, mass.per.particle.mg.max, mass.per.particle.mg,
     #           max.size.ingest.mm, max.size.ingest.um)) %>% 
  
     ### ------- Organism properites (mouth opening) ------------ ###
  #jamm et al (2020) Nature equation
      mutate(max.size.ingest.mm = case_when(
        max.size.ingest.reported.estimated == "reported" ~ max.size.ingest.mm,  # If "reported", use the calculated value
        max.size.ingest.reported.estimated == "estimated" ~ 10^(sim_beta_log10_body_length * log10(body.length.cm) - sim_body_length_intercept) * 10)
        ) %>%  
  mutate(max.size.ingest.um = 1000 * max.size.ingest.mm) %>% 
 ...
 #  
 #  #### -----Particle surface area, volume, mass ---- ######
 ...
 #                                               shape_f == "Fiber" & is.na(size.width.um.used.for.conversions) ~ SAfnx_fiber(width = 15, length = size.length.um.used.for.conversions), #assum 15 um width as operationally defined fibers in (kooi et al 2021 - main manusript)
 ...
 #  mutate(density.g.cm3 = as.numeric(density.g.cm3)) %>% #not sure why this was char, but fixing
 ...
 #  mutate(dose.mg.L.master.converted.reported = as.character(dose.mg.L.master.converted.reported)) %>% 
 ...
 #  mutate(dose.mg.kg.sediment.master.converted.reported = as.character(dose.mg.kg.sediment.master.converted.reported)) %>% 
 ...
 #  mutate(dose.particles.mL.master.converted.reported = as.character(dose.particles.mL.master.converted.reported)) %>% 
 ...
 #  mutate(dose.particles.kg.sediment.master.converted.reported = as.character(dose.particles.kg.sediment.master.converted.reported)) %>% 
...
##--- environmental calculations ---###
  ...
  #drop_na(particles.L.food.dilution,
   #       particles.L.ox.stress)
```
#### Checks
if some values are getting dropped, make simple df with  just variables used in alignments to determine why
```{r}
#simple variables
simple_vars <- c("rowid", "body.length.cm", "size.length.min.mm.measured", "size.length.min.mm.nominal", "shape_f",
  "size.length.max.mm.measured", "size.length.max.mm.nominal", "size.length.min.um.used.for.conversions",
  "size.width.min.um.used.for.conversions", "size.height.min.um.used.for.conversions",
  "size.length.max.um.used.for.conversions", "size.width.max.um.used.for.conversions",
  "size.height.max.um.used.for.conversions", "density.g.cm3", "particle.surface.area.um2", "particle.volume.um3", "dose.mg.L.master",
  "dose.particles.mL.master", "dose.particles.kg.sediment.master", "dose.mg.kg.sediment.master",
  "dose.mg.L.master.converted.reported", "dose.mg.kg.sediment.master.converted.reported",
  "dose.particles.mL.master.converted.reported", "dose.particles.kg.sediment.master.converted.reported",
  "particle.surface.area.um2.min", "particle.surface.area.um2.max", "particle.volume.um3.min", "particle.volume.um3.max",
  "mass.per.particle.mg", "dose.um3.mL.master", "dose.um3.kg.sediment.master",
  "dose.um2.mL.master", "dose.um2.kg.sediment.master", "dose.um2.ug.mL.master", "dose.um2.ug.kg.sediment.master",
  "dose.particles.mL.master", "max.size.ingest.um", "EC_mono_p.particles.mL_trans", "mu.p.mono",
  "EC_poly_p.particles.mL_trans", "CF_bio_trans", "EC_env_p.particles.mL_trans", "x_LL_sa_trans", "x_UL_sa_trans", "mu.sa.poly_trans",
  "polydispersity", "EC_poly_sa.particles.mL_trans", "EC_env_sa.particles.mL_trans", "EC_mono_p.particles.mL_ingest",
  "mu.p.poly_ingest", "EC_poly_p.particles.mL_ingest", "CF_bio_ingest", "EC_env_p.particles.mL_ingest", "x_LL_v_ingest", "x_UL_v_ingest",  "mu.v.poly_ingest", "EC_poly_v.particles.mL_ingest", "EC_env_v.particles.mL_ingest", "particles.mL.ox.stress",
  "particles.mL.food.dilution")

#simple_vars <- c("rowid", "body.length.cm", "max.size.ingest.um")
                 
aoc_d_simple <- aoc_deterministic %>% 
  select(all_of(simple_vars)) %>% 
  mutate(across(where(is.factor), as.character))

aoc_final_simple <- aoc_final %>% 
  ungroup() %>% 
  select(all_of(simple_vars)) %>% 
  mutate(across(where(is.factor), as.character))
```
...
left_join(aoc_d_simple %>% select(c(rowid, particles.mL.ox.stress, particles.mL.food.dilution)),
  ...
  mutate(ox.stress.dif = particles.mL.ox.stress.x - particles.mL.ox.stress.y,
         food.dif = particles.mL.food.dilution.x - particles.mL.food.dilution.y) %>% 
  filter(ox.stress.dif != 0)
```
...
aoc_final <- aoc_intermediate  %>% 
  ungroup() %>%
     ...
     ### ------- Organism properites (mouth opening) ------------ ###
  #jamm et al (2020) Nature equation
      mutate(max.size.ingest.mm = 10^(sim_beta_log10_body_length * log10(body.length.cm) - sim_body_length_intercept) * 10) %>%  
  mutate(max.size.ingest.um = 1000 * max.size.ingest.mm) %>% 
 ...
 #Calculate particle surface area
  ...
  mutate(dose.um2.ug.mL.master = dose.um2.mL.master / (mass.per.particle.mg / 1000)) %>% #correct mg to ug
  mutate(dose.um2.ug.kg.sediment.master = dose.um2.kg.sediment.master/(mass.per.particle.mg / 1000)) %>% 
  mutate(dose.particles.L.master = dose.particles.mL.master * 1000) %>% 
...
mutate(x2M_trans = case_when(is.na(max.size.ingest.um) ~ upper.tissue.trans.size.um, 
 ...
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.L_trans = dose.particles.L.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly_trans = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M_trans, #upper ingestible size limit (width of particle)
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.L_trans = (EC_mono_p.particles.L_trans * mu.p.mono)/mu.p.poly_trans) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio_trans = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M_trans, #upper translocatable
                        x1D = x1D_set, #default
                        ...
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.L_trans = EC_poly_p.particles.L_trans * CF_bio_trans) %>%  #aligned particle effect concentraiton (1-5000 um)
...
##--- environmental calculations ---###
  ...
  mutate(mu.sa.mono = case_when(
   ...
   #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.L_trans = (EC_mono_p.particles.L_trans * mu.sa.mono)/mu.sa.poly_trans) %>%  
  #calculate environmentally realistic effect threshold
 ...
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.L_ingest = dose.particles.L.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly_ingest = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M_ingest, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.L_ingest = (EC_mono_p.particles.L_ingest * mu.p.mono)/mu.p.poly_ingest) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio_ingest = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M_ingest, #upper ingestible length
                        x1D = x1D_set, #default
                        ...
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.L_ingest = EC_poly_p.particles.L_ingest * CF_bio_ingest) %>%  #aligned particle effect concentraiton (1-5000 um)
...
##--- environmental calculations ---###
  ...
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.L_ingest = (EC_mono_p.particles.L_ingest * mu.v.mono)/mu.v.poly_ingest) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.L_ingest = EC_poly_v.particles.L_ingest * CF_bio_ingest) %>% 
   ###### CLEANUP #####
  mutate(particles.L.ox.stress = EC_env_sa.particles.L_trans,
         particles.L.food.dilution = EC_env_v.particles.L_ingest) %>% 
  # some values are empty (this is a problem that we need to address - there shouldn't be so many)
  drop_na(particles.L.food.dilution,
          particles.L.ox.stress)
...
#store results in a list
 # Store results in a list
  MC_results[[i]] <- list(particles_L_ox_stress = aoc_final$particles.L.ox.stress,
                          particles_L_food_dilution = aoc_final$particles.L.food.dilution,
                          rowid = aoc_final$rowid)
}
```
...
results_df <- do.call(rbind, lapply(MC_results, function(x) {
  data.frame(
    rowid = x$rowid,
    particles_L_ox_stress = x$particles_L_ox_stress,
    particles_L_food_dilution = x$particles_L_food_dilution
  )
}))
...
aoc_MC <- left_join(aoc_MC, 
                    aoc_final %>% ungroup() %>%  
                      select(c(rowid, particles.mL.food.dilution, particles.mL.ox.stress)) %>%
...
mouth_cv_food <- aoc_MC %>% 
  ...
  ggpmisc::stat_poly_eq(aes(label = paste(..eq.label..), group = 1),
                        ...
                        method = "lm",
                        label.x = 0.1,
                        label.y = 0.7,  # Position at the top, adjust if necessary
                        parse = TRUE) +
   ggpmisc::stat_poly_eq(aes(label = paste(..rr.label..), group = 1),
                        ...
                        method = "lm",
                        label.x = 0.1,
                        label.y = 0.65,  # Position at the top, adjust if necessary
                        parse = TRUE) +
    ggpmisc::stat_poly_eq(aes(label = ..p.value.label.., group = 1),
                        ...
                        method = "lm",
                        label.x = 0.1,
                        label.y = 0.6,  # Position at the top, adjust if necessary
                        parse = TRUE) +
    xlab("Coefficient of Variation (unitless)") +
...
filtered.data.small.default_t1.2_MC <- aoc_risk_paper_MC %>% 
         ...
         mutate(dose_new_particles_L = particles.L.ox.stress) %>% 
...
filtered.data.large.default_t1.2_MC <- aoc_risk_paper_MC %>% 
  ...
  filter(Group != "Algae") %>% 
         ...
         mutate(dose_new_particles_L = particles.L.food.dilution) %>% 
