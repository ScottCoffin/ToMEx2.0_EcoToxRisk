...
pSSD_plot <-  function(quantiles_df = quantiles_df, species_data = species_data, data_name = data_name,
                       environment_name = environment_name, tier_name = tier_name, ERM_name = ERM_name
                       ){
...
x_limits <- c(
  min(c(quantiles_df$iv, species_data$NOEC_log10), na.rm = TRUE),  # Minimum value from data
  max_species_value + 2  # Add two orders of magnitude to the maximum species value
)
...
ggplot() +
  ...
  geom_label_repel(
    ...
    alpha = 0.8,
    box.padding = 0.3,  # Add padding around the label box
    point.padding = 0.5,  # Add padding between points and labels
    segment.alpha = 0.5,  # Make connecting lines semi-transparent
    nudge_x = 0.5,
    max.overlaps = Inf  # Allow all labels to be displayed
  ) +
  geom_line(data = quantiles_df, aes(x = iv, y = mean), color = "firebrick4", size = 1) +
    # Dynamic x-axis for log-transformed data
  scale_x_continuous(
    name = "NOEC (particles/L [1 to 5,000 μm])",
  ...
  #cols4all::scale_color_discrete_c4a_cat("cols4all.area9d") +
  cols4all::scale_color_discrete_c4a_cat("seaborn.dark", name = "Organism Group") +
...
PNEC_plot <- function(pssd_results, hcx, data_name){
 ...
 # Calculate specific percentiles
  median_val <- quantile(PNEC, probs = 0.5, type = 1)
  p001_val <- quantile(PNEC, probs = 0.01, type = 1)
  p5_val <- quantile(PNEC, probs = 0.05, type = 1)
  p95_val <- quantile(PNEC, probs = 0.95, type = 1)
  p999_val <- quantile(PNEC, probs = 0.999999999999, type = 1)  # 99.9th percentile for x-axis limits
...
# global filters for all thresholds
  filter(shape_f != "Not Reported",
          poly_f != "Not Reported",
           ...
           Group != "Plant",
           effect.metric != "HONEC") #%>% 
  #filter(simulation_id == "sim1") #BETA TESTING ONLY!!!
...
generate_plots_and_summary <- function(tier,
                                       hcx_input = 0.05, #change to 0.10 for Tier4
                                       environment,
                                       erm, 
                                       results_df,
                                       sim = 1000, #
  ...
  # Step 4b: Generate PNEC plot
  PNEC_plot <- PNEC_plot(
    pssd_results = pssd_results,
    hcx = hcx_input,
    data_name = paste("Tier", tier, "(", erm, ")")
  )
  
  # Step 5: Extract PNEC values for table
  PNEC_summary <- PNEC_data_summary(
    pssd_results = pssd_results,
    hcx = hcx_input,
    data_name = paste("Tier", tier, "(", erm, ")")
  )
  ...
  ggsave(
    filename = paste0("tier", tier, "_", erm, "_", environment, "_arranged.jpg"),
    dpi = 300,
    path = output_path,
    plot = arranged_plot,
  ...
  # Return the arranged plot and summary
  return(list(
    plot = arranged_plot,
    summary = PNEC_summary
  ))
}
```
...
```{r}
# Define parameters
tiers <- c(1, 2, 3, 4)  # Tiers 1/2, 3, and 4
...
for (tier in tiers) {
  for (environment in environments) {
    for (erm in erms) {
      # Adjust hcx_input based on tier
      hcx_input <- ifelse(tier == 4, 0.10, 0.05)
...
cat(crayon::blue("PSSD++ complete for all ERM/Tier/Environments!"))
```
...
for (combination in names(results)) {
  # Extract the PNEC summary statistics
  PNEC_stats <- results[[combination]]$summary$stats
  
  # Reshape the stats data frame into long format
  PNEC_stats_long <- data.frame(
    Statistic = rownames(PNEC_stats),
    Value = as.numeric(PNEC_stats[, 1]) # Extract the first column (values)
  )
  
  # Add the combination name and split it into tier, environment, and ERM
  PNEC_stats_long$Combination <- combination
  PNEC_stats_long$Tier <- sub("_.*", "", combination) # Extract tier
...
PNEC_summary_table <- PNEC_summary_table[, c("Tier", "Environment", "ERM", "Statistic", "Value")]

# Sort the table by Tier, Environment, and ERM
PNEC_summary_table <- PNEC_summary_table[order(PNEC_summary_table$Environment, PNEC_summary_table$ERM, PNEC_summary_table$Tier), ]
...
PNEC_summary_table_wide <- PNEC_summary_table %>%
  pivot_wider(
    names_from = Statistic, # Use the Statistic column for new column names
