...
# join necessary metadata
# metadata <- aoc_aligned %>% 
#   mutate(unique_id = as.factor(unique_id)) %>% 
#   distinct(unique_id, af.noec, af.time, environment, tier_zero_risk_f, tier_zero_tech_f, bio_f,
#            Species, Group, )

# get assessment factor data for MC tox values
# results_annotated <- left_join(results_df, metadata, by = "unique_id")
...
```{r}
MCdf_food_marine_T2 <- prep_data(results_df %>% 
                                   filter(environment == "Marine"#,
                                          #simulation_id %in% c("sim1", "sim2", "sim3")
                                          ) %>% 
                                   mutate(dose_new_particles_L = particles_L_food_dilution) %>% 
                                   drop_na(dose_new_particles_L))

#Step 2: run pSSD
tier1_2_food_pssd_plus <- run_pSSD_analysis(
  data_matrices = MCdf_food_marine_T2,
  num_iterations = 1000,
  sim = SIM,
  cv_dp = CV.DP.food,  # Default coefficeint
  cv_uf = CV.UF,  # Example coefficient of variation for UF
...
#Step 3: prep for ggplot
tier1_2_food_pssd_plus_plot_prep <- prepare_plot_data(pSSD = tier1_2_food_pssd_plus$pSSD,
                                            data_matrices = MCdf_food_marine_T2,
                                            species_data_source = aoc_aligned)

#Step 4a: generate ggplot (pSSD)
tier1_2_food_pssd_plus_Plot <-  pSSD_plot(quantiles_df = tier1_2_food_pssd_plus_plot_prep$quantiles_df,
                                    species_data = tier1_2_food_pssd_plus_plot_prep$species_data,
                                    data_name = "Tier 2 (Food Dilution)")


#Step 4b: generate ggplot (PNEC)
tier1_2_food_pssd_plus_PNECPlot <-  PNEC_plot(pssd_results = tier1_2_food_pssd_plus,
                                    hcx = 0.05,
                                    data_name = "Tier 2 (Food Dilution)")

# Step 5: extract PNEC values for table
tier1_2_food_pssd_plus_PNEC_summary <- PNEC_data_summary(pssd_results = tier1_2_food_pssd_plus,
                                               hcx = 0.05,
                                               data_name = "Tier 2 (Food Dilution)")

ggarrange(tier1_2_food_pssd_plus_Plot,tier1_2_food_pssd_plus_PNECPlot,
          ncol = 1)
```
