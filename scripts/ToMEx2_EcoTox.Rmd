...
cov_results <- results_df %>%
  group_by(rowid) %>%
  summarise(sd_ox_stress = sd(particles_L_ox_stress, na.rm = TRUE),
            mean_ox_stress = mean(particles_L_ox_stress, na.rm = TRUE),
            CoV_ox_stress = sd_ox_stress / mean_ox_stress,
            #food dilution
            sd_food_dilution = sd(particles_L_food_dilution, na.rm = TRUE),
            mean_food_dilution = mean(particles_L_food_dilution, na.rm = TRUE),
            CoV_food_dilution = sd_food_dilution / mean_food_dilution
  )
...
filtered.data.small.default_t1.2_MC <- aoc_risk_paper_MC %>% 
         ...
         mutate(dose_new = particles.L.ox.stress) %>%   
         drop_na(dose_new) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um),
         shape_f != "Not Reported",
         poly_f != "Not Reported",
...
filtered.data.large.default_t1.2_MC <- aoc_risk_paper_MC %>% 
  ...
  filter(Group != "Algae") %>% 
         ...
         mutate(dose_new = particles.L.food.dilution) %>% 
         drop_na(dose_new) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set),
         poly_f != "Not Reported",
         !environment %in% c("Terrestrial", "Not Reported"),
