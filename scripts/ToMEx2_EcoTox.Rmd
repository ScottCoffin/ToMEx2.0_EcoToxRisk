...
```{r}
getwd()
formatted_thresholds <- cbind(formatted_marine_base, formatted_freshwater_base)
...
pSSD_plot <- function(quantiles_df, species_data, data_name, environment_name, tier_name, 
  ...
  ggplot() +
    ...
    geom_label_repel(
      ...
      alpha = 0.8,
      box.padding = 0.4,  # Add padding around the label box
      point.padding = 0.8,  # Add padding between points and labels
      segment.alpha = 0.6,  # Make connecting lines semi-transparent
      nudge_x = -0.5,
      max.overlaps = Inf,  # Allow all labels to be displayed
      max.time = 5,  # Allow up to 5 seconds to resolve overlaps
      max.iter = 10000,  # Allow up to 10,000 iterations
      force = 2 #increase repulsion between labels
    ) +
    ...
    labs(title = "Microplastics Probabilistic Species Sensitivity Distribution",
         subtitle = paste0(ERM_name, " (", tier_name, ", ", environment_name, ")")) +
...
PNEC_plot <- function(pssd_results, hcx, data_name, 
 ...
 # Calculate specific percentiles
  ...
  label_data <- data.frame(
    x = c(p5_val, median_val, p95_val),
  #  y = c(0.00001, 0.00001, 0.00001),  # Adjust y position for labels
    label = c(paste0("**5th %** <br>", scales::comma(p5_val, 2)),
              paste0("**Median** <br>", scales::comma(median_val, 2)),
              paste0("**95th %** <br>", scales::comma(p95_val, 2)))
  )
...
generate_plots_and_summary <- function(tier,
                                       ...
                                       sim = 1000, # Default number of simulations
                                       num_iterations = 1000, # Default number of iterations
                                       cv_dp = NULL, 
  ...
  # Arrange plots for HC5
  arranged_plot_05 <- ggarrange(
    pSSD_plot, PNEC_plot_05,
    ncol = 1, heights = c(0.6, 0.4)
  )
  
  # Arrange plots for HC10
  arranged_plot_10 <- ggarrange(
    pSSD_plot, PNEC_plot_10,
    ncol = 1, heights = c(0.6, 0.4)
  )
  
  # Save the arranged plots
  ggsave(
    filename = paste0("tier", tier, "_", erm, "_", environment, "_arranged_HC5.jpg"),
    dpi = 300,
    path = output_path,
    plot = arranged_plot_05,
    width = 10, height = 8, units = "in"
  )
  
  ggsave(
    filename = paste0("tier", tier, "_", erm, "_", environment, "_arranged_HC10.jpg"),
    dpi = 300,
    path = output_path,
    plot = arranged_plot_10,
    width = 10, height = 8, units = "in"
  )
  ...
  return(list(
    ...
    arranged_plot_05 = arranged_plot_05,
    arranged_plot_10 = arranged_plot_10,
...
ggplot() +
  ...
  scale_fill_manual(name = "Method", 
                    values = c("PSSD++" = "coral", "MC-SSD" = "lightblue")) +
