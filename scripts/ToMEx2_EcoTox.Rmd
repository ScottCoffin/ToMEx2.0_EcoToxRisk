...
n_sim <- 10000 #probably should do 10,000 for final assessment, but keep number low for beta-testing 
...
aoc_MC_test <- aoc_intermediate  %>% 
  ...
  #Monte Carlo
  mutate(max.size.ingest.mm.sim = 10^(sim_beta_log10_body_length * log10(body.length.cm) - sim_body_length_intercept) * 10) %>% #jamm et al (2020) Nature
...
##### Archive
The following chunks contain beta testing code that could not be resolved. Keep for reference purposes.
```{r eval=FALSE, include=FALSE}
...
#   
# # calculate ERM for each species
# aoc_deterministic <- aoc_intermediate_alphas  %>% 
#   ungroup() %>%
...
#   #      TRUE ~ 10^(sim_beta_log10_body_length * log10(body.length.cm) - sim_body_length_intercept) * 10) #otherwise, estiamte from body length
#   #       ) %>%  
#   # mutate(max.size.ingest.um = 1000 * max.size.ingest.mm) %>% 
# 
#  #     ########### particle properties #############
...
##### Current
This chunk is starting from scratch.
```{r}
#source("functions.R") # necessary for surface area, volume calculations
  # #parameters to iterate
    #alpha <- median(alpha_samples)
...
#simple_vars <- c("rowid", "body.length.cm", "max.size.ingest.um", "x2M_ingest", "particles.mL.food.dilution")
#simple_vars <- c("rowid", "particles.mL.food.dilution", "particles.mL.ox.stress")
...
#join alpha values for each data point
aoc_intermediate_alphas_iter <- aoc_intermediate %>% 
  mutate(alpha = alpha) %>% 
  mutate(a.sa = a.sa) %>% 
   mutate(a.v =  a.v) %>% 
   mutate(a.m =  a.m) %>% 
...
# calculate ERM for each species
  aoc_MC_iter <- aoc_intermediate_alphas_iter  %>% 
    filter(size.length.um.used.for.conversions >= x1M_set) %>% 
    ungroup() %>% 
   #### TISSUE TRANSLOCATION ####
# define upper size length for Translocation 
#set to 83um for upper limit or max size ingest, whichever is smaller
...
##--- environmental calculations ---###
  ...
  mutate(particles.mL.ox.stress = EC_env_sa.particles.mL_trans,
         particles.mL.food.dilution = EC_env_v.particles.mL_ingest) %>% 
    drop_na(c(particles.mL.ox.stress,
            particles.mL.food.dilution))
...
example.df <-  left_join(results_df,
          aoc_MC %>% select(c(rowid,
                              ...
                              size.length.um.used.for.conversions, shape_f,
                              poly_f)),
          by = "rowid")

#get a few rowids to select for plotting
example.rowid.ox.stress.max <- example.df %>% 
  arrange(desc(CoV_ox_stress)) %>% 
  slice(1) %>% 
  pull(rowid)
  

example.rowid.ox.stress.min <- example.df %>% 
  arrange(CoV_ox_stress) %>% 
   slice(1) %>% 
  pull(rowid)
  
example.rowid.food.max <- example.df %>% 
    filter(Group != "Algae") %>% 
  arrange(desc(CoV_food_dilution)) %>% 
  slice(1) %>% 
  pull(rowid)

example.rowid.food.min <- example.df %>% 
    filter(Group != "Algae") %>% 
  arrange(CoV_food_dilution) %>% 
   slice(1) %>% 
  pull(rowid)
  
# Function to create ggplot for given rowid and description
create_ggplot_ox_stress <- function(rowid_input, plot_type) {

df_example <- example.df %>% filter(rowid == rowid_input)
...
example.CoV = round(df_example$CoV_ox_stress,2)
...
create_ggplot_food_dilution <- function(rowid_input, plot_type) {
...
example.group = as.character(df_example$Group[1])
example.particle.length = comma(df_example$size.length.um.used.for.conversions[1])
example.CoV = round(df_example$CoV_food_dilution,1)
example.max.size.ingest = comma(df_example$max.size.ingest.um[1])
...
plot_ox_stress_max <- create_ggplot_ox_stress(example.rowid.ox.stress.max,  "High CoV (Tissue Translocation)")
plot_ox_stress_min <- create_ggplot_ox_stress( example.rowid.ox.stress.min, "Low CoV (Tissue Translocation)")
plot_food_max <- create_ggplot_food_dilution( example.rowid.food.max, "High CoV (Food Dilution)")
plot_food_min <- create_ggplot_food_dilution(example.rowid.food.min, "Low CoV (Food Dilution)")
