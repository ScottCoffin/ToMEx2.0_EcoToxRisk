...
all_tiers_data <- bind_rows(
  ...
  mutate(
    y_position = as.numeric(factor(Tier)) + 
      (as.numeric(factor(Environment)) - 1.5) * 0.4
  )
...
final_plot <- ggplot() +
  ...
  geom_point(
    ...
    aes(x = PNEC, y = as.numeric(factor(Tier)) + 
          (as.numeric(factor(Environment)) - 1.5) * 0.4, fill = Environment),
    size = 5,
    shape = 22,
    color = "white",
    alpha = 0.95,
  ...
  # Labels and titles
  labs(
    title = "Comparison of Monte-Carlo [mcSSDs] and Deterministic [dSSD] Thresholds",
    subtitle = "Violin Plots = mcSSDs; Diamonds/Error-Bars = dSSD; All Tiers",
    fill = "Environment",
    color = "Environment",
