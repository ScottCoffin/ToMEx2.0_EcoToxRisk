...
Attempted to get this running on parallel but gave up....
<!-- ##### Parallel Processing -->
...
<!-- num_cores <- min(4, detectCores())  # Adjust the number of cores as needed -->
...
<!-- # Start timing -->
<!-- tictoc::tic("PSSD++ Parallel Processing Begins...") -->
...
<!--   .packages = c("tictoc", "crayon", "tidyverse",  -->
<!--                 "doParallel", "doSNOW", "ggrepel", "scales",  -->
<!--                 "ggpubr", "psych", "Matrix", "mc2d", "trapezoid", "ggtext", -->
<!--                 "reshape2", "sciscales"),  # Add any other required packages here -->
<!--   .options.snow = opts, -->
<!--   .export = c("run_pSSD_analysis", "rmore", "prepare_plot_data", "prepare_data", "prep_data", -->
<!--               "pSSD_plot", -->
<!--               "PNEC_data_summary", "Mode_Y", "make_PNEC_plot", "has_minimum_species", "get_results_df", -->
<!--               "generate_structure_checks", "generate_plots_and_summary", "generate_color_palette", "do.pSSD", -->
<!--               "do.pSSD_mod"), -->
<!--   .combine = list -->
<!-- ) %dopar% { -->
<!--   # Extract parameters for this iteration -->
<!--   tier <- param_combinations$tier[i] -->
<!--   environment <- param_combinations$environment[i] -->
<!--   erm <- param_combinations$erm[i] -->
<!--  # Generate the combination name -->
<!--   combination_name <- paste0("Tier", tier, "_", environment, "_", erm) -->
...
<!--     num_iterations = num_iterations, -->
<!--     quantile_type = quantile_type, -->
<!--     cv_dp = NULL,  # Always NULL as per the updated requirements -->
<!--     cv_uf = cv_uf, -->
<!--     species_data_source = species_data_source, -->
<!--     output_path = output_path, -->
<!--     presentation_path = presentation_path -->
...
<!--   # Return a named list for this combination -->
<!--   list(combination_name = result) -->
<!-- } -->
...
<!-- cat(crayon::blue(paste("PSSD++ complete for all ERM/Tier/Environments!", pSSD_time$callback_msg))) -->
<!-- ``` -->
...
plot_list <- list(
 ...
 labs(title = "Marine", y = "Tissue Translocation") + # Explicitly set y-axis label
...
SSD_base_thresholds <- base_thresholds_df %>% 
  mutate(y_position_comparison = case_when(
    Environment == "Marine" & ERM == "Tissue Translocation" ~ 1.7,
    Environment == "Marine" & ERM == "Food Dilution" ~ 0.8,
    Environment == "Freshwater" & ERM == "Tissue Translocation" ~ 4.1,
    Environment == "Freshwater" & ERM == "Food Dilution" ~ 1.3
  ))
...
#define xlimits
xlimits <- c(quantile(pSSD_PNECs$PNEC, 0.001),
             quantile(pSSD_PNECs$PNEC, 0.9999))
  
ggplot() +
  ###### SSD PNECs ####
  geom_point(data = SSD_PNECs, 
              aes(x = PNEC, y = y_position_comparison, fill = "SSD", color = "SSD"),
              size = 5, shape = 22, 
              color = "white",
              alpha = 0.95,
              stroke = 1.8 #thickness of outline
   ...
   geom_errorbarh(data = SSD_PNECs, 
                  aes(xmin = PNEClcl, xmax = PNECucl, y = y_position_comparison, fill = "SSD", color = "SSD"),
                  height = 0.2,
                  alpha = 0.75,
                  linewidth = 1.2) +
  ###### pSSD PNECs #####
  # geom_histogram(data = pSSD_PNECs, aes(x = PNEC, y = ..density.., fill = "PSDD++", color = "PSSD++"), 
  #                 bins = 350, alpha = 0.3) +
  geom_density(data = pSSD_PNECs, aes(x = PNEC, color = "PSSD++", fill = "PSSD++"), lwd = 1, 
               alpha = 0.5) +  # Density line for pSDD++
  
  ###### MC-SSD PNECs #####
  # geom_histogram(data = MC_PNECs, aes(x = PNEC, y = ..density.., fill = "MC-SSD", color = "MC-SSD"), 
  #                 bins = 350, alpha = 0.3) +
  geom_density(data = MC_PNECs, aes(x = PNEC, color = "MC-SSD", fill = "MC-SSD"), lwd = 1, alpha = 0.5
               ) +  # Density line for MC-SSD
  
  # Adjust x-axis limits and scale
  xlim(quantile(MC_PNECs$PNEC, 0.001), quantile(MC_PNECs$PNEC, 0.999)) +
  scale_x_log10(limits = xlimits,
                labels = label_sci(trailing = FALSE)) +
...
tiers <- c("Tier3")
erms <- c("Tissue Translocation", "Food Dilution")
