...
generate_color_palette <- function(data) {
  ...
  palette <- cols4all::c4a("powerbi.high_contrast", n = length(unique_groups)) # Generate colors
...
pSSD_plot <- function(quantiles_df, all_NOEC, species_data, data_name, environment_name, tier_name, 
                      ERM_name, color_palette = color_palette) {
...
legend_labels <- setNames(
  ...
  # Calculate the dynamic x-axis limits
  max_species_value <- max(species_data$NOEC_log10, na.rm = TRUE)  # Find the maximum species value

  x_limits <- c(
    min(c(quantiles_df$iv, species_data$NOEC_log10), na.rm = TRUE) - 1,  # Minimum value from data + 2 orders of magnitude
    max_species_value + 6  # Add 5 orders of magnitude to the maximum species value
  )
  
  ggplot() +
    # this code would make a ribbon around the quantiles for each point in hte PSSD. This is the 'default' for these plots, but we've opted instead to show the distribution around each data point due to alignments. see geom_points below
    # geom_ribbon(data = quantiles_df, aes(x = iv, ymin = min, ymax = max), fill = "coral", alpha = 0.05) +
    # geom_ribbon(data = quantiles_df, aes(x = iv, ymin = q05, ymax = q95), fill = "coral", alpha = 0.2) +
    # geom_ribbon(data = quantiles_df, aes(x = iv, ymin = q25, ymax = q75), fill = "coral", alpha = 0.6) +
    
    # display each individual study point's distribution (with size/alpha related to probability according to MC alignment)
    geom_point(data = all_NOEC,
    ...
    # only add points for geomean (preferred to show all points as above)
    # geom_point(data = species_data, aes(x = NOEC_log10, y = Prop, color = Group)) +
    ### provide label for median value (lower 60% of points)
    geom_label_repel(
      data = species_data %>% filter(Prop <0.6),
      aes(x = NOEC_log10, y = Prop, label = Species, color = Group),
     direction = "y",
     hjust = 0,
     nudge_x =  7,
      fontface = "italic",  # Make labels bold
      size = 3,  # Increase label size
      fill = "white",  # Add a white background to labels
      alpha = 0.85,
      segment.alpha = 0.4,  # Make connecting lines semi-transparent
      max.overlaps = Inf,  # Allow all labels to be displayed
      max.time = 5,  # Allow up to 30 seconds to resolve overlaps
      max.iter = 1e7#,  # Allow up to 1,000,000 iterations
     # force = 4, #increase repulsion between labels
    ) +
### provide label for median value (upper 40% of points)
    geom_label_repel(
      data = species_data %>% filter(Prop >=0.6),
      aes(x = NOEC_log10, y = Prop, label = Species, color = Group),
     direction = "y",
    ...
    geom_line(data = quantiles_df, aes(x = iv, y = mean), color = "black", size = 1) +
    scale_x_continuous(
      breaks = pretty(x_limits, n = 6),  # Dynamically generate breaks
      labels = function(x) parse(text = paste0("10^", x)),  # Back-transform labels to powers of 10
      limits = x_limits  # Dynamic limits with one order of magnitude added
    ) +
    ...
    scale_fill_manual(values = color_palette, 
                      labels = legend_labels,
                       name = "Organism Group") + # Apply consistent color mapping
    guides(color = guide_legend(override.aes = list(shape = 21, size = 3, label = "", hjust = NULL, color = NA, alpha = 1)),
           fill =  guide_legend(override.aes = list(shape = 21, size = 3, label = "",  hjust = NULL, color = NA, alpha = 1,
                                                    fill = unname(color_palette)))
           ) +  # Combine color and fill legends
    
    labs(title = "Microplastics PSSD++",
...
color_palette <- generate_color_palette(all_species)
...
generate_plots_and_summary <- function(tier,
  ...
  # Step 1: Prepare data
  MCdf <- prep_data(results_df)
  
  # Step 2: Run pSSD analysis
  pssd_results <- run_pSSD_analysis(
    data_matrices = MCdf,
  ...
  # create df of the PNECs
  PNEC_05 <- apply(pssd_results$pSSD, 2, 
                   function(x) quantile(x,
                                        probs = 0.05, #HC value for tier3
                                        type = quantile_type
                                        ))
  PNEC_05_df <- data.frame(PNEC = PNEC_05)
  
  # Step 4b: Generate PNEC plots for both HCX values
  PNEC_plot_05 <- make_PNEC_plot(
  ...
  # Step 5: Extract PNEC values for both HCX values
  ...
  # Save the arranged plots for the manuscript
  ggsave(
  ...
  # Save the arranged plots for presentations (wide format)
  ggsave(
    filename = paste0(erm, "_", environment, "_arranged_plot.jpg"),
    dpi = 450,
  ...
  # Return the plots and summaries for both HCX values
  return(list(
    pSSD_plot = pSSD_plot,
...
cache_dir <- "../output/cache_pssd_results_300sim_trapezoidal"  # directory to store cache
#cache_dir <- "../output/cache_pssd_results_300sim_lognormal"

sim <- 300 # Number of simulations to run (tried higher values - but takes extremely long time!!)
num_iterations <- sim
cv_uf <- 0.5
...
```{r}
saveRDS(results, 
        "../data/output/PSDDplusplusresults.rds")
```
...
# Save the final plot
ggsave(
  filename = "../output/Manuscript_Figs/final_pSSD_plot.jpg",
  plot = arranged_plot,
  dpi = 500,
  width = 9,
  height = 7,
...
##### Comparison of pSSD and SSD-MC Distributions
```{r}
...
scale_x_log10(
  ...
  labs(title = environment, y = "Relative Density", x = "PNEC (Tier 3; Particles/L [1 to 5,000 μm])")
...
# Save the final plot
ggsave(
  filename = "../output/Manuscript_Figs/PNEC_compare_arranged_plot.jpg",
