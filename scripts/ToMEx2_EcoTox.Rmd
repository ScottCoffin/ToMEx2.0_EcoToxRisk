...
#### Just studies used in Thresholdsd
```{r}
effect_metrics_used <- aoc_z_summary_start %>% 
   pivot_longer(
  ...
  mutate("Freshwater (Tier 1/2)" = as.character(Freshwater_thresholds1_2),
         "Freshwater (Tier 3/4)" = as.character(Freshwater_thresholds3_4),
         "Marine (Tier 1/2)" = as.character(Marine_thresholds1_2),
         "Marine (Tier 3/4)" = as.character(Marine_thresholds3_4)) %>% 
  select(-c(Freshwater_thresholds1_2, Freshwater_thresholds3_4,
            Marine_thresholds1_2, Marine_thresholds3_4)) %>% 
  mutate(effect.metric = case_when(
    is.na(effect.metric) ~ "Intermediary exposure concentration",
    T ~ effect.metric
  )) %>% 
...
# particle shape breakdown
particle_shapes_used <- aoc_z_summary_start %>% 
    pivot_longer(
    cols = c(thresholds1_2, thresholds3_4),
    names_to = "threshold",
    values_to = "status"
...
# polymer breakdown
polymer_breakdown_used <- aoc_z_summary_start %>% 
    pivot_longer(
    cols = c(thresholds1_2, thresholds3_4),
    names_to = "threshold",
    values_to = "status"
...
# size breakdown
size_breakdown_used <- aoc_z_summary_start %>% 
   pivot_longer(
    cols = c(thresholds1_2, thresholds3_4),
    names_to = "threshold",
    values_to = "status"
...
# group breakdown
group_breakdown_used <- aoc_z_summary_start %>% 
    pivot_longer(
    cols = c(thresholds1_2, thresholds3_4),
    names_to = "threshold",
    values_to = "status"
...
data_used_thresholds_summary <- aoc_z_summary_start %>% 
  ...
  rename("Freshwater (Tier 1/2)" = V1,
         "Marine (Tier 1/2)" = V2,
         "Freshwater (Tier 3/4)" = V3,
         "Marine (Tier 3/4)" = V4
         ) %>% 
  dplyr::slice(-c(1,2)) %>% 
  bind_rows(effect_metrics_used,
            group_breakdown_used,
            size_breakdown_used,
            particle_shapes_used,
            polymer_breakdown_used) %>% 
  rownames_to_column("Factor") %>% 
