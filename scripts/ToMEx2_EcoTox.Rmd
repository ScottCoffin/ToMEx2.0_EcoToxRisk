...
make_PNEC_plot <- function(pssd_results, hcx, data_name, 
 ...
 # Calculate specific percentiles
  median_val <- quantile(PNEC, probs = 0.5, type = quantile_type)
  p001_val <- quantile(PNEC, probs = 0.01, type = quantile_type)
  p5_val <- quantile(PNEC, probs = 0.05, type = quantile_type)
  p95_val <- quantile(PNEC, probs = 0.95, type = quantile_type)
  p999_val <- quantile(PNEC, probs = 0.999999999999, type = quantile_type)  # 99.9th percentile for x-axis limits
...
```{r warning=FALSE}
# Define parameters
tiers <- c(3)  # It really only makes sense to make tier 3
...
# Example coefficients and data sources
sim <- 300 #number of simulations in pSSD. 3 is minimum. I've tried 1,000, but it creates a matrix too big for my RAM (112 GB). It appears that we need to keep this value less than 500 or so for most machines to be able to run (300 seems to be a sweet spot).
num_iterations <- sim
cv_uf <- 0.5 # Default value for uncertainty factors
quantile_type = 8 # type 1 is actual data (doesn't work for small datasets),
# type 6 is mean-unbiased estimate (hybrid between discrete data picking and linear interpolatiuon),
# ype 7 is linear interpolation (smooths data)
# type 8 is linear interpolation for the median-unbiased esimate (i -1/3) / (n + 1/3)

# provide mapping for species/group/environment
...
iteration_times <- numeric(total_iterations) # Store time for each iteration
...
for (tier in tiers) {
  for (environment in environments) {
    for (erm in erms) {
      ...
      # Store results
      results[[paste0("Tier", tier, "_", environment, "_", erm)]] <- result
      
      # End timing this iteration
      iteration_end_time <- Sys.time()
      iteration_time <- as.numeric(difftime(iteration_end_time, iteration_start_time, units = "secs"))
      iteration_times[iteration_count] <- iteration_time # Store the time
      
      # Calculate the running average time
      running_average_time <- mean(iteration_times[1:iteration_count])
      
      # Print progress every 100 iterations or at the end
      if (iteration_count %% 1 == 0 || iteration_count == total_iterations) {
        cat(Sys.time(),
            crayon::yellow(sprintf(
          "Iteration %d/%d complete. Running average time per iteration: %.2f seconds. Estimated time remaining: %.2f seconds.\n",
          iteration_count, total_iterations, running_average_time,
          running_average_time * (total_iterations - iteration_count)
        )
        ))
      }
    }
