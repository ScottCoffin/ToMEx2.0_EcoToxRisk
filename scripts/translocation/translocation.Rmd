...
```{r}
getwd()
joined <- read_excel("../../data/input/translocation_scored_2.xlsx")
```
...
where 𝛽0  is the intercept and 𝛽1 is the slope coefficient from the fitted model.
...
sim_X50_trim <- sim_X50 %>% as.data.frame() %>% filter(.>0)

summary(sim_X50)

# Plotting the distribution of X50

x50_MC_hist <- ggplot(data = data.frame(X50 = sim_X50), aes(x = X50)) +
  geom_histogram(bins = 200, fill = "steelblue", color = "black", linewidth = 0.10) +
  scale_x_log10()+
  xlim(c(1,1000))+
  labs(title = "Distribution of Simulated X50 Values",
       x = "Particle Length (μm) of 50th Percentile Probability For Tissue Translocation",
       y = "Frequency") +
