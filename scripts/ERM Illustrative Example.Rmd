...
In this example of a monodisperse effect concentration for *C.dubia*, $EC_{mono}$ is the average particle count per volume that caused the effect in the exposure study, and $\mu_{x,mono}$ is the mean value of the ERM of interest in this study, which in the case of particle count, is 1. 
```{r}
POD <- POD %>% 
  mutate(mu.p.mono = 1)
```
...
aoc_z_filtered %>% select(translocatable_poly,
                          ...
                          dose.particles.L_trans,
                          dose.particles.L_ingest
                          ) %>% 
  mutate_if(is.numeric, ~signif(., 3)) %>% 
  drop_na(translocatable_poly) %>% 
  distinct() %>% 
...
POD <- POD %>% 
  mutate(mu.p.mono = 1) %>% 
  mutate(EC_poly_p.particles.L_ingest = (dose.particles.L * mu.p.mono ) / mu.p.poly_ingest,
         EC_poly_p.particles.L_trans = (dose.particles.L * mu.p.mono ) / mu.p.poly_trans)
...
POD <- POD %>% 
  ...
  # calculate min and max volume when polydisperse particles are used
...
cat("x_UL_v_ingest = ", comma(POD$x_UL_v_ingest[1],3), "um^3 \n",
...
```{r}
POD <- POD %>% 
  mutate(EC_poly_v.particles.L_ingest = (dose.particles.L * mu.v.mono)/mu.v.poly_ingest,
         EC_poly_v.particles.L_trans = (dose.particles.L * mu.v.mono)/mu.v.poly_trans)   
...
POD <- POD %>% 
  ...
  mutate(particle.surface.area.um2 = SAfnx(length = size.length.um.used.for.conversions,
                                           ...
                                           R = R.ave,
                                           H_W_ratio = H_W_ratio)) %>% 
  # calculate min/max SA for polydisperse mixtures
...
```{r}
POD <- POD %>% 
  mutate(EC_poly_sa.particles.L_trans = (dose.particles.L  * mu.sa.mono)/mu.sa.poly_trans,
         EC_poly_sa.particles.L_ingest = (dose.particles.L  * mu.sa.mono)/mu.sa.poly_ingest)  
...
```{r}
POD <- POD %>% 
  mutate(EC_poly_m.particles.L_ingest = (dose.particles.L * mu.v.mono)/mu.v.poly_ingest,
         EC_poly_m.particles.L_trans = (dose.particles.L * mu.v.mono)/mu.v.poly_trans)   
